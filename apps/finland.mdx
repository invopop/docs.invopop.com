...
<Tabs>
	<Tab title="Description">
		...
		This app covers the **domestic operator network**. Invopop has partnered with [Apix Messaging](https://www.apix.fi), an accredited Finnish operator: you send one GOBL invoice carrying the receiver's e-invoice address, and it is delivered as a compliant e-invoice in the format the receiver's operator expects — Finvoice, TEAPPSXML, or another.

		For a Finnish receiver on Peppol, use the [Peppol app](/apps/peppol) instead. The two apps divide by network, so pick the one that matches how your receiver is reachable. Look the receiver up at [verkkolaskuosoite.fi](https://verkkolaskuosoite.fi), which since 2024 lists addresses on both networks, so one lookup tells you which app you need.

		Before anything is sent, this app checks how the receiver would be reached on the domestic network and refuses the invoice unless the answer is a networked e-invoice, naming the channel that would have been used instead.
		...
		* **Domestic-network delivery:** Receivers on the Finnish operator network are reached in the format their operator expects, resolved per receiver.
	...
	<Tab title="Actions">
		...
		<Card title="Register party in Finland" icon="https://assets.invopop.com/flags/fi.svg" horizontal>
		 ...
		 Provision the party its own operator account and activate its services. A Finnish party is also allocated its e-invoice address and can receive; a party established elsewhere is registered to send only. A party must be approved and registered before it can send.
		</Card>
	...
	<Tab title="Documents">
		<AccordionGroup>
			<Accordion title="Finland supplier">
				...
				<Supplier />
				A supplier established outside Finland can also send through this app, carrying its own tax ID and its own electronic address — for a Swedish company, its `0007` Peppol identifier. Those suppliers can send only; see the [supplier registration guide](/guides/fi-finvoice-supplier#suppliers-outside-finland).
			</Accordion>

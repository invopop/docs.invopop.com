...
import Supplier from '/snippets/parties/sa/supplier.mdx';
import Customer from '/snippets/parties/sa/customer.mdx';
...
<Tabs>
	...
	<Tab title="Documents">
		...
		<AccordionGroup>
			<Accordion title="ZATCA supplier">
				A supplier with the 15-digit VAT registration number (starts and ends with `3`) and the CRN commercial registration identity ZATCA requires.
				<Supplier />
			</Accordion>
			<Accordion title="ZATCA customer">
				A business customer identified by a TIN identity and a full Saudi national address.
				<Customer />
			</Accordion>
		</AccordionGroup>

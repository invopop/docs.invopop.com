<AccordionGroup>    
    <Accordion title="🇵🇱 Invopop resources for Poland">
|    |    |
|-----------|------------|
|  Compliance |  <Icon icon="https://assets.invopop.com/flags/pl.svg" /> [Invoicing compliance in Poland](/compliance/poland)<br /> <Icon icon="timeline" /> [Compliance timeline](/timelines/poland)|
|  Apps |  <Icon icon="https://assets.invopop.com/apps/ksef/icon.svg" /> [KSeF Poland](/apps/ksef-poland) |
|  Guides | <Icon icon="book" /> [Poland Guide](/guides/pl-ksef) |
|  FAQ | <Icon icon="square-question" /> [Poland FAQ](/faq/poland) |
|  GOBL | <Icon icon="https://assets.invopop.com/icons/gobl.svg" />  [Poland Tax Regime](https://docs.gobl.org/regimes/pl)<br /> <Icon icon="https://assets.invopop.com/icons/gobl.svg" /> [Polish KSeF FA_VAT Addon](https://docs.gobl.org/addons/pl-favat-v3) |
    </Accordion>
</AccordionGroup>

...
import B2BPrepaymentMin from '/snippets/invoices/pl/ksef-b2b-prepayment.min.mdx';
import B2BPrepayment from '/snippets/invoices/pl/ksef-b2b-prepayment.mdx';
import B2BSettlementMin from '/snippets/invoices/pl/ksef-b2b-settlement.min.mdx';
import B2BSettlement from '/snippets/invoices/pl/ksef-b2b-settlement.mdx';
import B2CInvoiceMin from '/snippets/invoices/pl/ksef-b2c-standard.min.mdx';
import B2CInvoice from '/snippets/invoices/pl/ksef-b2c-standard.mdx';
import SelfBilledMin from '/snippets/invoices/pl/self-billed.min.mdx';
import SelfBilled from '/snippets/invoices/pl/self-billed.mdx';
...
<Accordion title="B2B Standard Invoice">
	...
	- the [`pl-favat-v3`](https://docs.gobl.org/addons/pl-favat-v3) addon ensures the document will be validated using the KSeF rules built into the [GOBL](https://docs.gobl.org) library,
...
<Accordion title="B2B Credit Note">
	...
	In this example, we're issuing a credit note to correct or refund a previous invoice. In Poland, every correction is a _faktura korygująca_ (corrective invoice) — there is no separate credit note document type.

	Notice:

	- the `type` field is set to `credit-note` to indicate this is a corrective document,
	- the `preceding` field references the original invoice being corrected, including its KSeF number as a `favat-ksef-number` stamp,
	- the minimal version contains only the essential fields; calculations are handled by GOBL,
	- the built version shows `"pl-favat-invoice-type": "KOR"` automatically set in the tax extensions, mapping to the FA(3) corrective invoice type,
	- quantities and amounts are expressed as positive values, as in every GOBL credit note; they are inverted wherever negative values are expected, such as in the FA(3) XML, and,
	- the document is validated using the KSeF correction rules to ensure compliance with Polish requirements.

...
<AccordionGroup>
	<Accordion title="Example service invoice (B2B)">
		...
		- optionally, we've set the [`gr-mydata-branch`](https://docs.gobl.org/addons/gr-mydata-v1#branch-number) extension on the supplier to report the branch (establishment number in the AADE registry) issuing the invoice; when absent, branch `0` (headquarters) is reported, and the same extension can be set on the customer, and,

...
<AccordionGroup>
	...
	<Accordion title="AT return guide">
		...
		- the document carries the [`return`](https://docs.gobl.org/addons/pt-saft-v1#delivery-tags) tag, which the [`pt-saft-v1`](https://docs.gobl.org/addons/pt-saft-v1) addon normalizes to the [`pt-saft-movement-type`](https://docs.gobl.org/addons/pt-saft-v1#movement-type) extension `GD` (Guia de Devolução); you can also set the extension explicitly,
	...
	<Accordion title="AT return from global waybill">
		...
		- as in the standalone return guide, the `return` tag is applied (or, equivalently, the `pt-saft-movement-type` extension is set to `GD`),

...
<Accordion title="How do I know if an invoice I sent was accepted or rejected?">
  When the customer answers with an OIOUBL ApplicationResponse, it arrives through the same import workflow as received invoices and becomes its own `bill/status` silo entry, with the original XML attached. The import job's result carries the outcome — `accepted` or `rejected` — so the workflow can branch on it; the import template marks the entry's state accordingly.
</Accordion>

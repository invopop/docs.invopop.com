  ...
  <Accordion title="How do I issue invoices from a branch (υποκατάστημα)?">
    Set the [`gr-mydata-branch`](https://docs.gobl.org/addons/gr-mydata-v1#branch-number) extension on the supplier party to the branch's establishment number from the AADE registry. When the extension is absent, invoices report the headquarters branch (`0`). The same extension works on the customer party. See the branch invoice example in the [myDATA invoicing guide](/guides/gr-iapr#send-an-invoice).
  </Accordion>

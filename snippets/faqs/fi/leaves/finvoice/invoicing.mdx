...
<Accordion title="What happens if the sending account runs out of credit?">
  Deliveries through the operator are prepaid. When the sending account's balance runs out, the job fails under its own `no-saldo` code and retries — it completes once the account is topped up.
</Accordion>

  ...
  <Accordion title="My company already uses another Plateforme Agréée for one flow. Can I register with Invopop for another?">
    Yes — this is a common setup, for example a company handling AR with one PA and AP with another. Run the [registration workflow](/guides/fr-pa-registration) with a directory entry suffix of your choice for the flow you want Invopop to handle, such as `SIREN_AR`. Invoices and statuses addressed to an entry are delivered to the platform that registered it, so the two flows never conflict and act fully independently.
  </Accordion>

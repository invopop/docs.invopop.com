  <Accordion title="Why is my invoice processing taking so long?">
    The **Send invoice to SDI** step completes as soon as SDI accepts the file — it doesn't wait for SDI's verdict. What takes time is SDI's own processing: a rejection usually arrives within minutes, but delivery confirmations can take up to 5 days. Each verdict is recorded against the invoice as a status entry, so nothing is stuck while you wait. See the [status guide](/guides/it-sdi-status) for the full lifecycle.
  </Accordion>
  <Accordion title="How do I know if my invoice was delivered successfully?">
    SDI confirms delivery with an `RC` (*ricevuta di consegna*) notification, recorded on the invoice as a status entry with the GOBL status `acknowledged`. A rejection arrives as `NS` with status `error`. Every notification and what it means is covered in the [status guide](/guides/it-sdi-status).
  </Accordion>
  <Accordion title="How do I test different SDI outcomes in the sandbox?">
    Set a reserved *codice destinatario* on the customer — for example `SIMNS00` to simulate a rejection. The sandbox simulates SDI end to end and feeds back the notifications a real exchange would produce; any ordinary code resolves to a successful delivery. The full list of reserved codes is in the [issuing guide](/guides/it-sdi-invoicing#testing-in-the-sandbox).
  </Accordion>

  <Accordion title="How do I add codice fiscale and Partita IVA?">
    Both Italian tax identifiers have their own place on a party. The *partita IVA* (VAT number) goes in `tax_id`, and the *codice fiscale* goes in `identities` under the key `it-fiscal-code`. A party may carry both.

    An Italian supplier always needs a *partita IVA*. An Italian customer needs at least one of the two. Which identifier a given customer needs, and what the conversion fills in when it has none, is covered in the [issuing guide](/guides/it-sdi-invoicing#identifying-the-parties).

    Here's an example showing both identifiers on a supplier:
    ```json Supplier with Partita IVA and Codice Fiscale
    {
        "supplier": {
            "name": "MªF. Services",
            "tax_id": {
                "country": "IT",
                "code": "12345678903"
            },
            "identities": [
                {
                    "key": "it-fiscal-code",
                    "code": "MRTMTT91D08F205J"
                }
            ]
        }
    }
    ```
  </Accordion>

  <Accordion title="What if the customer has no tax ID at all?">
    SDI still accepts the invoice. A customer with no identifier of its own is addressed with the placeholder SDI expects — `0000000` for a foreign private individual — and the conversion fills that in for you, so there is nothing to supply by hand and no placeholder of your own to invent.

    Certified archiving is stricter: it needs a real VAT number or *codice fiscale* for both parties and fails with `party has no usable fiscal ID`. So an invoice to an unidentified customer transmits but will not preserve. If you archive what you issue, collect an identifier first. See [identifying the parties](/guides/it-sdi-invoicing#identifying-the-parties).
  </Accordion>

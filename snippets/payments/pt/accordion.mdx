...
<AccordionGroup>
	<Accordion title="AT payment receipt">
		...
		- this is a `receipt` payment with a single entry in the `methods` array using `cash`, which causes GOBL to automatically assign the [`pt-saft-payment-means`](https://docs.gobl.org/addons/pt-saft-v1#payment-means) extension to `NU` (Numerário). Use additional entries in `methods` to record several payment means with their own amounts on the same receipt,

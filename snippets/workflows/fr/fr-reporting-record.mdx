---
title: "France record document for reporting"
description: "Records an invoice or payment as a Flux 10 row for France e-reporting"
countryCode: "fr"
appId: "gov-fr"
---
```json France record document for reporting workflow
{
    "name": "France record document for reporting",
    "description": "Records an invoice or payment as a Flux 10 row for France e-reporting",
    "steps": [
        {
            "id": "753fdc8d-80ce-4a3f-a8ca-c01158e64cf9",
            "name": "Set state",
            "provider": "silo.state",
            "summary": "Set state to `processing`{.state .processing}",
            "config": {
                "state": "processing"
            }
        },
        {
            "id": "7d204bc6-f42d-4f55-b08a-fca3c19bee15",
            "name": "Record document for reporting",
            "provider": "gov-fr.reporting.record"
        },
        {
            "id": "d0660d9a-31b0-4428-b9da-bef14f7ae200",
            "name": "Set state",
            "provider": "silo.state",
            "summary": "Set state to `registered`{.state .registered}",
            "config": {
                "state": "registered"
            }
        }
    ],
    "rescue": [
        {
            "id": "7ec292ea-36f2-49bb-b2ce-bb97bd6a1782",
            "name": "Set state",
            "provider": "silo.state",
            "summary": "Set state to `error`{.state .error}",
            "config": {
                "state": "error"
            }
        }
    ]
}
```

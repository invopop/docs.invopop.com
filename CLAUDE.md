...
- `scripts/` — small shell scripts that maintain `snippets/` (`fill-empty-faqs.sh` for FAQ placeholders, `build-workflow-data.sh` to regenerate workflow `-data.mdx` files from their `.mdx` source).
...
- `snippets/workflows/<country|integration>/` — reusable workflow descriptions. Every workflow embedded by a guide comes in a pair: `<name>.mdx` holds the fenced ` ```json ` block (rendered via default snippet import in the guides' JSON/Code tab, also embedded by apps pages), and `<name>-data.mdx` holds the same JSON as `export const <unique>Workflow = {…}` and **nothing else**. Guides import the data (`import { <unique>Workflow } from '…-data.mdx'`) and feed it to `<WorkflowDiagram workflow={…} />` from `snippets/components/workflow.jsx`, which renders a console-style step list (branches, notes, state chips via `<Badge>`, per-provider icons from its `providerIcons` map). **The `.mdx` file is the single hand-edited source: never edit a `-data.mdx` — run `scripts/build-workflow-data.sh` to regenerate them all (`--check` verifies; pass a source path to create a new companion).** Hard-won constraints on the data snippet: it must contain only the export (a snippet mixing markdown content and const exports compiles to an `undefined` stub on a fresh build and the whole section breaks — warm dev servers mask this); export names must be globally unique (never `workflow`; convention: camelCase of `<dir>-<file>` + `Workflow`); import without aliasing (`{ x as y }` double-declares in Mintlify's inlining). Mintlify's built-in components (`Badge`, etc.) are in scope inside custom snippet JSX without imports. Beware that Mintlify only compiles Tailwind classes written as static `className` literals; dynamically-assembled class strings silently don't style.

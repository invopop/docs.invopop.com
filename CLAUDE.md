...
- `snippets/tools/` — self-contained interactive helpers, each an `<Accordion>` wrapping a component from `snippets/components/` plus a `<CodeGroup>` carrying the same logic as copyable code. The code block is not redundant: custom JSX doesn't survive into the `.md` export, so it is the only version agents reading the docs can use. Currently `fr-test-party.mdx` (Chorus Pro test SIREN → GOBL party), imported by `guides/fr-chorus-pro-supplier.mdx` and `snippets/invoices/fr/chorus-pro-accordion.mdx`.

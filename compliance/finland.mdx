...
The diagram below shows how an outbound invoice reaches the buyer through the operator, on whichever network the buyer lives:
...
sequenceDiagram
    ...
    participant AX as Operator (Apix)
    participant B as Buyer
    S->>IV: Invoice (GOBL)
    IV->>IV: Convert to Peppol BIS 3.0 (UBL)
    IV->>AX: Submit to the operator
    alt Buyer on the domestic network
        AX->>B: Deliver as Finvoice
    else Buyer on Peppol
        AX->>B: Deliver over Peppol
    end
```
...
<Steps>
  ...
  <Step title="Generate the e-invoice">
    Invopop converts the invoice to Peppol BIS Billing 3.0 (UBL 2.1), whichever network the buyer is on.
  </Step>
  <Step title="Submit to the operator">
    Invopop submits the invoice to Apix Messaging, the accredited Finnish operator it partners with, which resolves how the buyer is reachable and refuses anything that would not arrive as a true e-invoice.
  </Step>
  <Step title="Deliver on the domestic network">
    If the buyer receives through the domestic operator network, the operator converts the invoice to Finvoice and delivers it there.
  </Step>
  <Step title="Deliver over Peppol">
    If the buyer is registered on Peppol, the invoice is delivered over Peppol instead.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Peppol Network (B2G)">
    ...
    <Card title="Finland" size="20" icon="https://assets.invopop.com/flags/fi.svg" href="/apps/finland" horizontal>
  ...
  <Accordion title="Domestic channels: Finvoice and TEAPPSXML">
    <Note>Invopop reaches domestic-network receivers through the [Finland App](/apps/finland): invoices are submitted as Peppol BIS Billing 3.0 UBL to Apix Messaging, an accredited Finnish operator, which converts them to Finvoice for receivers on this network.</Note>

...
description: "Argentina's mandatory e-invoicing requirements, Factura Electrónica implementation, and e-reporting obligations for B2B, B2C, and B2G transactions"
---

import ArgentinaResources from '/snippets/tables/argentina-resources.mdx';
import ComplianceFAQ from '/snippets/faqs/ar/composers/page-compliance.mdx';
...
<AccordionGroup>
  ...
  <Accordion title="Comprobante T (tourism sector, tax-free sales to non-residents)">

    Comprobante T (Factura T, Nota de Débito T, Nota de Crédito T) is used by hotels and resident travel agencies to document tourism services sold to non-resident tourists or non-resident travel agencies under Argentina's *régimen de reintegro de IVA al turismo extranjero*. These documents are submitted through the dedicated **WSCT (Web Service Comprobante T)** rather than the general WSFE service.

    |            |             |
    |------------|-------------|
    | **Models** | B2B, B2C (non-resident recipients) |
    | **Format** | XML (ARCA WSCT schema) |
    | **Infrastructure** | [ARCA WSCT](https://www.afip.gob.ar) |
    | **Model** | Clearance continuous transaction control (CTC) |
    | **Code** | 195 (Invoice T), 196 (Debit Note T), 197 (Credit Note T) |
    | **Scope & Deadline** | Mandatory for tourism operators issuing tax-free invoices to non-residents |
    | **Agency** | [ARCA](https://www.afip.gob.ar) |
    | **Invopop Support** | Available - [ARCA Supplier onboarding](/guides/ar-arca-suppliers), [ARCA Invoices](/guides/ar-arca-invoices) |

    Each Comprobante T must declare:

    - The **issuer-receiver relationship** (`ar-arca-tourism-type`), with values such as direct accommodation to a non-resident tourist, accommodation to a resident or non-resident travel agency, or travel-agency-to-travel-agency operations.
    - A **tourism item code** (`ar-arca-tourism-item`) on every line — for example hotel with breakfast, hotel without breakfast, or surplus.
    - The customer's address (required by WSCT) and a VAT rate of 21% on every line.
...
<AccordionGroup>
  ...
  <Accordion title="Invoice Classification">
    ...
    | **T** | Tourism operators (hotels, resident travel agencies) | Non-resident tourists and non-resident travel agencies |
  ...
  <Accordion title="WSFE Web Services">
    ...
    - **wsct** (Web Service Comprobante T): For type T (tourism) invoices issued to non-resident tourists and non-resident travel agencies

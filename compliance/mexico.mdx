...
| Pago    | `P` |  Payment receipt (REP) issued when collecting against a previously-issued PPD invoice. | Yes |
...
<AccordionGroup>
  ...
  <Accordion title="Pago (payment receipt)">
    ...
    **Invopop coverage:** Invopop issues type-P documents. Model the receipt as a GOBL [`bill.Payment`](https://docs.gobl.org/draft-0/bill/payment) referencing each PPD invoice being settled, and process it through the SAT payment workflow to stamp the CFDI de pago. See the [payment receipt example](/guides/mx-sat-issuing) in the issuing guide.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="PPD — Pago en Parcialidades o Diferido (pay in installments)">
    ...
    Invopop supports issuing both the PPD invoice and the subsequent REPs as payments are received.
  </Accordion>
</AccordionGroup>

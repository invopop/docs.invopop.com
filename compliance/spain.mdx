  ...
  * If any of the following apply, it must be mentioned: tax-exempt transaction, customer issues the invoice, reverse charge (customer pays the tax), Travel Agency special scheme, or second-hand goods special scheme.
  ...
  <Accordion title="Main special tax types for Spain">
    ...
    4. IRNR (Impuesto sobre la Renta de no Residentes)
      * Non-resident income tax withheld at source on payments to individuals or entities without a permanent establishment in Spain.
      * Regulated by Real Decreto Legislativo 5/2004 (TRLIRNR).
      * Rates depend on the type of income and on whether the recipient is resident in another EU/EEA Member State with an effective exchange of tax information (reduced rates may apply), so they are supplied per invoice rather than predefined.

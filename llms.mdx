...
A `200` returns the entry: `id`, `folder` (`invoices`, `contacts`, ...), `doc_schema`, a `snippet` summarising the document, `version` and `versions`, and the built envelope in `data`. `signed` and `state` are omitted until something sets them, and the deprecated `draft: true` marks an unsigned entry. Listing with `GET /silo/v1/entries?folder=invoices&limit=10` returns the same shape per item; `limit` must be between 10 and 100. Free-text search (`GET /silo/v1/search?q=`) covers series, codes, party names and tax IDs but not line items, and it is fed asynchronously: in testing a new entry took about 45 minutes to appear, so do not use search to check whether something you just created exists. Fetch it by ID or `key` instead.

...
|  |  |
|-------|-------------|
| **Upload logo** | Upload an image which will be used as a logo in the document. A square or horizontal logo layout in PNG format is recommended. If you need to upload a different logo for each supplier, include the `logos` field in the party entry [as detailed in the FAQ](/guides/pdf-invoice#how-can-i-add-a-different-logo-for-each-supplier). |
...
Make sure your image is publicly accessible. Sometimes server restrictions prevent our services from accessing the image. An easy way to verify this is by asking ChatGPT “is this URL https://… publicly accesible?”
</Accordion>

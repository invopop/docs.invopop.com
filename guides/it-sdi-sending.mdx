...
- The party registration workflow includes a step that enables archiving for the supplier. It requires the party to have a valid Italian tax ID. Re-running it on an already-enabled supplier reports `Skipped`.
...
- The import invoice workflow includes the same archive step after the Import step, so the invoices you receive are preserved too, recorded as received.

Preservation is asynchronous: the archive step stays `Queued` until the receipt is ready, then completes with `OK`. The receipt normally arrives within minutes but can take hours; the step keeps checking, less often as time goes on, and gives up only after several hours. If the preservation service fails partway through, the step retries from where it stopped without uploading the document again. Re-running it on an already-archived invoice reports `Skipped`, with nothing fetched or attached again.

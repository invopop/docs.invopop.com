...
After completing this registration with the tax authority, execute the [SDI register customer](#template-2) workflow in Invopop to complete the setup for that company.

<Note>
Complete both steps for each company: (1) Register Codice Destinatario `JKKZDGR` with the Italian tax authority, and (2) Run the Customer Registration workflow in Invopop. Registration is only needed for receiving invoices, not sending.
</Note>

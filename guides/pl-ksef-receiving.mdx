...
<Note>
  Invoices are downloaded from KSeF one by one, and KSeF allows 64 downloads per hour for each NIP in production (ten times more in test mode). Each sync can therefore import up to 64 invoices for every hour of its interval. If a sync finds more, it fails without importing them and asks you to sync more often. If you expect to receive more than 64 invoices an hour, [contact support](mailto:support@invopop.com).
</Note>
...
The sync workflow will query KSeF for all invoices received in that date range and create individual import jobs for each invoice found. The same download limit applies: a date range may hold up to 64 invoices per hour it covers, so split large backfills into shorter ranges.
...
<Tip>
  Common import issues include certificate expired or invalid, KSeF API temporarily unavailable, more invoices in a sync window than KSeF allows us to download, invoice format not supported by GOBL conversion, and network connectivity issues. Check your workflow error logs in the Console for detailed error messages.
</Tip>

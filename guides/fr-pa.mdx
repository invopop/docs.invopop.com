...
| | |
|------|------------|
| **Directory** (annuaire) | The French e-invoicing directory where parties must be registered to participate in regulated e-invoicing flows. Not to be confused with the Peppol Directory. |
...
### Registration — Directory (annuaire)

The _annuaire_ is France's national directory of e-invoicing endpoints. It records every registered party along with the PA (Plateforme Agréée) responsible for handling their invoices, so that senders know where to route invoices and the tax administration knows who is in scope for regulated e-invoicing flows.

<Note>
  From here on we refer to the annuaire simply as **the Directory**. This is _not_ the same as the [Peppol Directory](https://directory.peppol.eu/) — the Peppol Directory is an international lookup of Peppol participants, while the French Directory is the annuaire maintained by the French tax administration and used to determine whether a flow is regulated.
</Note>

The registration process prepares parties to send and receive invoices in France. It extends the standard [Peppol registration workflow](/guides/peppol) with one additional French-specific step to register the party in the Directory.

<Note>
  Peppol registration always enables **both sending and receiving**. Once a party is listed in the Directory, it must be reachable to receive invoices — there is no send-only option.
</Note>
...
<Tabs>
  <Tab title="Template">
    <Card iconType="duotone" title="France PA register party workflow" icon="code-branch" href="https://console.invopop.com/redirect/workflows/new?template=fr-fr-pa-register" cta="Add to my workspace">
      Registers a party in the French Directory and on the Peppol network for sending and receiving invoices.
    </Card>
  </Tab>
  <Tab title="Code">
...
The French PA specification lists five accepted invoice formats. In practice, three of them cover essentially all real-world exchanges, and Invopop supports sending _and_ receiving all three:

| Format | Description |
|---|---|
| **UBL** | XML invoice based on the Peppol France CIUS. The default format used across the Peppol network and the one we recommend for all outgoing flows. |
| **CII** | UN/CEFACT Cross Industry Invoice — another XML format, semantically equivalent to UBL and required by some counterparties (particularly in cross-border and automotive/industrial contexts). |
| **Factur-X** | Hybrid PDF/A-3 with an embedded CII XML payload. Human-readable on top, machine-readable underneath. Common with buyers who want a visual invoice alongside the structured data. |
...
Regardless of which format goes over the wire, the underlying invoice is always the **same GOBL document with the `fr-ctc-flow2-v1` add-on**. The format is only a wrapping step at the end of the workflow — switching format is a configuration change, not a data-model change. That means:
...
<Note>
  This workflow is for **B2B regulated flows**. The `gov-fr.invoice.record` step validates this up-front and will reject the invoice if it doesn't meet the criteria (e.g. if the receiver is not registered in the Directory).

  For mixed traffic, run a Directory lookup step and `if-else` statements earlier in the workflow and branch non-regulated invoices into the periodic e-reporting path instead.
</Note>
...
3. **Record Invoice** — validates SIRENs, checks both parties against the Directory, and records the invoice.
...
3. **Record Invoice** — the same `gov-fr.invoice.record` provider as the sending workflow, but run with `direction: "incoming"`. It validates SIRENs, checks both parties against the Directory, resolves _your_ registered party as the customer (or supplier if self-billed), and guards against duplicates.
4. **Set state to Registered** — marks the incoming invoice as successfully ingested.

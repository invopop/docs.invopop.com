...
For the dispute and refusal codes (`206`, `207`, `208`, `210`) the line carries a `reason`, and that's where the substance is:
...
The supplier-issued **`209` Complétée** takes no reason; use the line **`description`** to state what was supplied to lift the suspension.
...
<AccordionGroup>
  ...
  <Accordion title="209 — Complétée (Completed)">
    `bill.Status` · `type: update` · line `key: other`. Issued by the supplier to lift a suspension — the line **`description` states what was supplied** (e.g. the missing purchase order). No reason code.
    <State209 />
  </Accordion>

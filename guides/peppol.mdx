...
import { peppolSendWorkflow } from '/snippets/workflows/peppol/send-data.mdx';
import { peppolReceiveWorkflow } from '/snippets/workflows/peppol/receive-data.mdx';
import { peppolRegisterWorkflow } from '/snippets/workflows/peppol/register-data.mdx';
import { peppolUnregisterWorkflow } from '/snippets/workflows/peppol/unregister-data.mdx';
...
#### Validating the endpoint before sending

The **Validate endpoint** (`validate.endpoint`) action checks that a document can actually be delivered before attempting the send: both parties carry a Peppol inbox, the receiver is registered in the Peppol directory, and the receiver accepts the document type of the generated file.

The action returns `true` or `false` as its result code, so a workflow can branch to an alternative channel when Peppol delivery isn't possible.

  ...
  You can skip this step if you don't plan to issue invoices.
    ...
    <Tabs>
      ...
      <Tab title="Build from scratch">
          ...
          2. **Seal for the AT (Portugal)** - No configuration required.
          3. **Record for SAF-T reporting (Portugal)** - No configuration required.
          4. **Send to the AT (Portugal)** - No configuration required.
          5. **Generate PDF (Original)** - Portuguese - A4.
          6. **Generate PDF (Duplicate)** - Portuguese - A4.
          7. **Set State** to `Sent`.
  ...
  You can skip this step if you don't plan to issue payments.
    ...
    <Tabs>
      ...
      <Tab title="Build from scratch">
          ...
          2. **Seal for the AT (Portugal)** - No configuration required.
          3. **Record for SAF-T reporting (Portugal)** - No configuration required.
          4. **Generate PDF (Original)** - Portuguese - A4.
          5. **Generate PDF (Duplicate)** - Portuguese - A4.
          6. **Set State** to `Sent`.
...
The following examples show partial [GOBL](https://docs.gobl.org) documents you can copy and paste directly into the [Invopop Console](https://console.invopop.com) or store via the API as silo entries. After uploading, you must run the corresponding processing workflow (invoice, delivery, or payment) created during setup.
    ...
    <Tip>
      [GOBL](https://docs.gobl.org) Documents reported to the AT require the following properties to perform the correct normalization and validation steps. Invopop will try to assign these automatically, but we recommend defining them explicitly before uploading:
      - `$regime` to be set to [`PT`](https://docs.gobl.org/regimes/pt)
...
<Tabs>
  <Tab title="Invoice">
    ...
    - we've added the [`pt-saft-v1`](https://docs.gobl.org/addons/pt-saft-v1) addon; this ensures the document will be validated and normalized using the SAF-T PT v1 rules built into the [GOBL](https://docs.gobl.org) library,
    - the `supplier` and `series` fields are set to example values; you'll need to replace them with the details of supplier and series registered as per the previous sections. The series must be registered in the Supplier Portal and follow Portuguese naming conventions (see [Series Management](#series-management) for details),
    - this is a `standard` invoice and the advance payments don't cover the total amount of the invoice; so, [GOBL](https://docs.gobl.org) will automatically assign the [`pt-saft-invoice-type`](https://docs.gobl.org/addons/pt-saft-v1#invoice-type) extension to `FT` (Fatura),
    - the `card` payment method will cause the addon to set the [`pt-saft-payment-means`](https://docs.gobl.org/addons/pt-saft-v1#payment-means) extension to `CC` (Cartão crédito),
    - the `h` item unit will cause the [`pt-saft-product-type`](https://docs.gobl.org/addons/pt-saft-v1#product-type) extension to be set to `S` (Serviço),
    - the `standard` VAT rate will cause the [`pt-saft-tax-rate`](https://docs.gobl.org/addons/pt-saft-v1#tax-rate-code) extension to be set to `NOR` (Normal),
    - the [`pt-region`](https://docs.gobl.org/regimes/pt#region-code) extension will automatically be set to the default value of `PT`, and
  ...
  <Tab title="Delivery">
    ...
    - we've added the [`pt-saft-v1`](https://docs.gobl.org/addons/pt-saft-v1) addon; this ensures the document will be validated and normalized using the SAF-T PT v1 rules built into the [GOBL](https://docs.gobl.org) library,
    - the `supplier` and `series` fields are set to example values; you'll need to replace them with the details of supplier and series registered as per the previous sections. The series must be registered in the Supplier Portal and follow Portuguese naming conventions (see [Series Management](#series-management) for details),
    - this is an `advice` document type and the advance payments don't cover the total amount of the invoice; so, [GOBL](https://docs.gobl.org) will automatically assign the [`pt-saft-invoice-type`](https://docs.gobl.org/addons/pt-saft-v1#invoice-type) extension to `FT` (Fatura),
    - the `standard` VAT rate will cause the [`pt-saft-tax-rate`](https://docs.gobl.org/addons/pt-saft-v1#tax-rate-code) extension to be set to `NOR` (Normal),
    - the [`pt-region`](https://docs.gobl.org/regimes/pt#region-code) extension will automatically be set to the default value of `PT`, and
    - there are no totals or calculations; all these will be made automatically when uploading.
  </Tab>
  <Tab title="Payment">
    ...
    - we've added the [`pt-saft-v1`](https://docs.gobl.org/addons/pt-saft-v1) addon; this ensures the document will be validated and normalized using the SAF-T PT v1 rules built into the [GOBL](https://docs.gobl.org) library,
    - the `supplier` and `series` fields are set to example values; you'll need to replace them with the details of supplier and series registered as per the previous sections. The series must be registered in the Supplier Portal and follow Portuguese naming conventions (see [Series Management](#series-management) for details),
    - this is a `receipt` payment and the method is set to key `cash`; so, GOBL will automatically assign the [`pt-saft-payment-means`](https://docs.gobl.org/addons/pt-saft-v1#payment-means) extension to `NU` (Numerário),
    - the `h` item unit will cause the [`pt-saft-product-type`](https://docs.gobl.org/addons/pt-saft-v1#product-type) extension to be set to `S` (Serviço),
    - the `standard` VAT rate will cause the [`pt-saft-tax-rate`](https://docs.gobl.org/addons/pt-saft-v1#tax-rate-code) extension to be set to `NOR` (Normal),
    - the [`pt-region`](https://docs.gobl.org/regimes/pt#region-code) extension will automatically be set to the default value of `PT`, and
    - there are no totals or calculations; all these will be made automatically when uploading.
...
The **Send to the AT (Portugal)** step uses the AT's webservice to submit each document to the tax authority immediately after it is sealed and recorded.

---
title: "Portugal - AT"
description: Issue invoices and other documents in Portugal
---

## Introduction

Invopop's AT Portugal provider allows you to issue all types of tax documents in Portugal and submit them to the Autoridade Tributária e Aduaneira (AT) via SAF-T PT reports or the AT's real-time web service.

This guide will walk you through the process of registering a supplier, followed by the steps to issue invoices and other documents in their name.
...
At this point you can add any webhooks or notification steps that might be useful for your systems to receive after a supplier has successfully registered and is ready to issue documents.
...
Click the **Configure** button on the App, and choose the "Post-registration flow" workflow we created in the previous step, or leave empty if you don't want to use it.
...
Finally, we need to prepare the actual workflow that will be used to process invoices and other documents. For this example the workflow should be named something like "Issue AT document", and it will consist of the following steps:
...
Once the series are registered, the supplier will be able to issue documents using them. When this happens, the system will automatically execute the "Post-registration flow" workflow if one was configured.

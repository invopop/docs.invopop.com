---
title: "Correct invoices guide"
sidebarTitle: "Correct Invoices"
description: "Step by step guide to correct issued invoices with credit notes"
icon: "eraser"
---
...
<Prompt description="Have your agent issue a credit note against an existing entry through the API." icon="eraser" actions={["copy", "cursor"]}>
Read https://docs.invopop.com/guides/correct-invoice.md. Using the token in INVOPOP_TOKEN, fetch the silo entry I name with GET https://api.invopop.com/silo/v1/entries/ followed by its ID, confirm it is signed, and tell me its type, series, code and payable total. Then create the correction by posting to https://api.invopop.com/silo/v1/entries a body with previous_id set to that entry ID and a correct object with type credit-note and the series I give you. Do not build the credit note by hand. Show me the new entry's preceding block, run the workflow I name on the new entry, and report the result. If the regime requires a corrective invoice instead of a credit note, say so and stop.
</Prompt>

## Correcting your first invoice using a credit note

Before we dive in make sure you've completed the [Quickstart](/get-started/quickstart) tutorial, you should now have a processed invoice in your workspace.
...
## Using the console
  ...
  ## Using the API
    ...
    * **Auto-generate a credit note**: You reference an existing invoice, and Invopop builds the credit note for you.
    ...
    It's perfect if you already have the credit note prepared, whether you built it manually or generated it somewhere else.
    ...
    **How it works**
    ...
    ```
    Here's a complete example based on the PDF invoice we used earlier. Notice how it differs from the previous invoice example: the type is set to `credit-note`, a preceding block is included to reference the original invoice, and the `$schema` points to the credit note schema.
    ...
    Instead of defining everything yourself, you can simply reference the invoice you want to correct. Just pass the **Silo Entry ID** and the type of correction, and Invopop will take care of the rest.

...
Every FatturaPA invoice has to identify both parties fiscally, and Italy has two identifiers for the job: the *partita IVA* (VAT number, in `tax_id`) and the *codice fiscale* (in `identities`, under the key `it-fiscal-code`). Which one a party carries depends on what that party is.

An Italian supplier always needs a *partita IVA*. An Italian customer needs at least one of the two — a business normally has the VAT number, a private individual only the *codice fiscale*, and both may be present. A foreign customer needs neither.

Where a party has no identifier of its own, SDI expects a placeholder in its place. You never write those placeholders yourself: the conversion fills them in.

| Customer | What the FatturaPA carries |
|---|---|
| Italian, with *partita IVA* | `IdFiscaleIVA` — `IT` plus the code from `tax_id` |
| Italian, *codice fiscale* only | `CodiceFiscale`, and no `IdFiscaleIVA` |
| Foreign business inside the EU | `IdFiscaleIVA` with its own country and VAT number |
| Foreign business outside the EU | `IdCodice` `OO99999999999`, whatever its own tax number is |
| Foreign, no tax ID at all | `IdCodice` `0000000` — treated as a private individual |

Only EU VAT numbers mean anything to SDI, so a non-EU company's own tax number never reaches the XML — knowing the country is enough.

Routing follows the same principle, but keys off the country rather than the identifier. An Italian customer with no `it-sdi-code` inbox is addressed as `0000000`, which tells SDI to deliver through whatever channel the recipient registered for itself. A customer whose `tax_id` names any country other than Italy is addressed as `XXXXXXX` — always, even when it carries an inbox, because SDI has no way to deliver outside Italy. Send that invoice to the customer yourself, as a PDF.

<Accordion title="Foreign private customer example">
  A country and no tax number. The conversion writes `IdCodice` `0000000` for the identity and addresses the delivery as `XXXXXXX`, because the customer is not Italian.
  <ItCustomerForeign />
</Accordion>

<Warning>
  A customer carrying no fiscal identifier converts and sends, but **cannot be archived**. Certified archiving needs a real VAT number or *codice fiscale* for both parties and fails with `party has no usable fiscal ID` — the placeholders that satisfy SDI do not satisfy it. If you archive what you issue, collect an identifier before issuing rather than relying on the placeholder.
</Warning>

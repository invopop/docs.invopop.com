...
import SendInvoiceWorkflow, { itSendInvoiceWorkflow } from '/snippets/workflows/it/italy-send-invoice.mdx';
import ImportStatusWorkflow, { itImportStatusWorkflow } from '/snippets/workflows/it/italy-import-status.mdx';
...
Every invoice has to identify both parties fiscally, and Italy has two identifiers for the job: the *partita IVA* (VAT number) and the *codice fiscale*. Which one you provide depends on who the party is.

The supplier always needs a *partita IVA*, in `tax_id`. For the customer:
...
| An Italian private individual | `tax_id` with `country: "IT"` and no code, plus an `it-fiscal-code` identity holding their *codice fiscale*. No inbox |
| Anyone outside Italy | `tax_id` with their country. Include their VAT number if they are in the EU — elsewhere the country is all SDI uses. No inbox |

An Italian customer needs at least one of the two identifiers and may carry both. SDI cannot deliver outside Italy, so send foreign customers the invoice yourself, as a PDF.

The country always lives in its own field, so none of these codes carries a country prefix: write a *partita IVA* as `09876543217`, not `IT09876543217`. Send the prefix anyway and it is stripped when the invoice is built. A *codice destinatario* is 7 characters; a personal *codice fiscale* is 16, while a company's is the same 11 digits as its VAT number.
...
<Note>
  When you don't have a code for a party, leave it out rather than inventing a placeholder. SDI expects particular filler values in these cases and the conversion writes them for you: `0000000` where the customer has no tax number, and `XXXXXXX` as the delivery address for anyone outside Italy. An `it-sdi-code` inbox on a foreign customer is ignored for the same reason.
</Note>

<Warning>
  If you archive your invoices, collect a real identifier before issuing. Certified archiving needs a VAT number or *codice fiscale* for both parties, and the filler values that satisfy SDI don't satisfy it — the invoice transmits, then fails preservation with `party has no usable fiscal ID`.
</Warning>

...
import ItCustomerAccordion from '/snippets/parties/it/accordion.mdx';
...
<Prompt description="Have your agent issue an invoice through SDI in your sandbox." icon="file-invoice" actions={["copy", "cursor"]}>
Read https://docs.invopop.com/guides/it-sdi-invoicing.md, https://docs.invopop.com/llms.md and https://docs.gobl.org/addons/it-sdi-v1.md. In my sandbox workspace, using the token in INVOPOP_TOKEN and the workflow ID I give you, create a bill/invoice entry with $regime IT, $addons containing it-sdi-v1, the supplier I describe, and a customer of the kind I describe. Start from one of the example invoices in the guide, and do not invent tax codes, recipient codes or extension codes. Run the workflow with wait=30, then fetch the entry and report its state, the job's per-step statuses and faults, and the files attached. Then open the FatturaPA XML and tell me what is in CodiceDestinatario, PECDestinatario and the CessionarioCommittente identifiers, and why each value is what it is.
</Prompt>
...
|---|---|
| An Italian business | `tax_id` with `country: "IT"` and the *partita IVA*, plus an `it-sdi-code` inbox holding their *codice destinatario*, an `it-sdi-pec` inbox holding their PEC address, or both |
...
Here is each case as a customer [party](https://docs.gobl.org/draft-0/org/party):

...
A company established in **Sweden, Denmark, Norway or Estonia** can be registered here to invoice Finnish receivers. It is registered with its own country's tax ID, and the operator files it under its national business number. State that number as a GOBL identity on the party — type `ON` for a Swedish Org.nr or a Norwegian organisasjonsnummer, `CVR` for a Danish CVR, and a legal-scope identity for an Estonian registry code. Where the party carries none, it is read off the VAT number: that works for Sweden, Denmark and Norway, whose VAT numbers contain the business number, but not for Estonia, where the VAT number stands in unless the registry code is stated. Nothing else about sending changes — the same workflow, the same delivery check before each invoice, the same formats on the wire.

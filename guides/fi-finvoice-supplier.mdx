...
- **The party's details**: legal name, Business ID (*y-tunnus*, e.g. `2617416-4`), and a Finnish address. The Business ID is uploaded without its hyphen as the GOBL `tax_id.code`. A supplier established outside Finland carries its own country's tax ID instead — see [suppliers outside Finland](#suppliers-outside-finland).
...
- **Reception workflows configured**: registration checks that the Finland app has a sync and an import workflow configured, and fails with "reception not configured" otherwise. Suppliers outside Finland don't receive here, so they are not held to this. 
...
Registration allocates the party's address on the operator's side: it is the party's own OVT code, the same one you set as the `0216` inbox on the supplier party. Counterparties will ask for it, and in Finland an e-invoice address is quoted as a pair, the address plus the operator that serves it. Give them both: your OVT code and Invopop's operator, **Apix Messaging, operator ID `003723327487`**.
...
A company established **anywhere outside Finland** can be registered here to invoice Finnish receivers. It is registered with its own country's tax ID, which is also what the operator files it under: the VAT number as is for most countries, and the bare Org.nr for Sweden and Norway, whose VAT numbers contain it. Nothing else about sending changes — the same workflow, the same delivery check before each invoice, the same formats on the wire.

These parties are registered for **sending only**. A Finnish e-invoice address belongs to a Finnish Business ID, so a company established elsewhere is not listed in the [national registry](https://verkkolaskuosoite.fi) and cannot be reached over the Finnish network. No inbox is polled for it, and the authorization agreement its representative signs covers sending alone.

To receive as well, register the party in the [Peppol app](/apps/peppol): any Finnish sender able to invoice abroad reaches it there.

<Note>
  Sending through this app puts the invoice on Finland's domestic operator network. The [Peppol app](/apps/peppol) is the other route — it reaches Finnish organisations registered on Peppol, though not the ones that only receive on the domestic network.
</Note>
...
As soon as the authorization is approved, the workflow resumes on its own. Registration finds or creates the party's company under Invopop's partner account at the operator, activates its services, and waits until the operator reports every service in force and the party's e-invoice address allocated. Until then the job stays queued rather than reporting success on an account that can't yet receive. A supplier outside Finland has no Finnish address to wait for, so its registration completes as soon as it can send.
</Step>
</Steps>

Once registration completes, a Finnish party can send and receive e-invoices, and its inbox at the operator starts being polled automatically. A supplier outside Finland can send.

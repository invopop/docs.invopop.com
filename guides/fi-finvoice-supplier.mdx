...
A company established in **Sweden, Denmark, Norway or Estonia** can be registered here to invoice Finnish receivers. It is registered with its own country's tax ID, and the operator files it under its national business identifier: Sweden's Org.nr, Denmark's CVR, Norway's organisasjonsnummer, Estonia's VAT number. Nothing else about sending changes — the same workflow, the same delivery check before each invoice, the same formats on the wire.

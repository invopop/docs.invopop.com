...
A company established in **Sweden, Denmark, Norway or Estonia** can be registered here to invoice Finnish receivers. It is registered with its own country's tax ID, which is also what the operator files it under: the VAT number as is for most countries, and the bare Org.nr for Sweden and Norway, whose VAT numbers contain it. Nothing else about sending changes — the same workflow, the same delivery check before each invoice, the same formats on the wire.

These parties are registered for **sending only**. A Finnish e-invoice address is derived from a Business ID, so a company without one is given no address, is not listed in the Finnish registry, and cannot receive over the Finnish network. Its registration completes without an address, no inbox is polled for it, and the authorization agreement its representative signs covers sending alone.
...
<Note>
  Suppliers from other countries can't be registered in this app. To invoice Finnish receivers from elsewhere, use the [Peppol app](/apps/peppol) — it reaches the Finnish organisations registered on Peppol, though not the ones that only receive on the domestic network.
</Note>

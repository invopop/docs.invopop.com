...
description: Register Finnish parties with the Apix operator so they can send and receive e-invoices.
---
...
Finland has no government clearance system: e-invoices travel through accredited operators, and every sender and receiver works through an operator account. Invopop has partnered with [Apix Messaging](https://www.apix.fi), an accredited Finnish operator, and the [Finland app](/apps/finland)'s registration provisions each of your Finnish parties its **own** account with Apix, activates its services, and allocates its e-invoice address.
...
<Info>Registration requires the reception workflows to be configured first, because a registered party's inbox starts being polled right away. Create the sync and import workflows and have them set on the app as described in the [receiving guide](/guides/fi-finvoice-receiving#setup), then come back here.</Info>
</Step>
...
Create a silo entry with the party's details. The example below shows the fields a Finnish party carries — for registration, the name and the y-tunnus are what matters:
...
Registration finds or creates the party's company under Invopop's partner account at Apix, activates its services, and waits until the operator reports every service in force and the party's e-invoice address allocated. Until then the job stays queued rather than reporting success on an account that can't yet receive.
</Step>
</Steps>

...
When a match is found, the step writes the resolved `0225` electronic address back onto the looked-up party (supplier or customer for invoices and payments, or the root party for standalone party documents). Downstream steps in the workflow can then route the document without having to repeat the lookup.
...
A SIREN may have multiple entries (one per registered SIRET / electronic address), so the response is always a list. An empty `results` array means the SIREN is not currently registered in the Annuaire.

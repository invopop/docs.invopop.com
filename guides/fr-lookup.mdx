...
A SIREN may have multiple entries (one per registered SIRET / electronic address), so the response is always a list. For each unique party, the lookup returns only the latest directory line and only when that line is an active deposit — parties whose most recent line cancels a prior registration are treated as unregistered. An empty `results` array means the SIREN is not currently registered in the Annuaire.

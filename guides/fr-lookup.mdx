...
* A live workspace (in Sandbox, API Lookup will error with `403 Forbidden` while workflows return OK for any valid SIREN).
...
In the **send document workflow**, the step can be configured to lookup suppliers and customers. The job will fail if no matching entry is found, and you can handle the exception any way you want (usually routing to non-regulated flow).
...
<Note>
  French overseas territories that sit outside the French VAT zone (Guyane, Mayotte, New Caledonia and French Polynesia) are treated as foreign zones for the e-invoicing reform.

  Annuaire lookup returns no result even when they carry a French SIREN, so their transactions are always routed to [e-reporting](/guides/fr-pa-reporting). 
</Note>
...
- **One or both parties unregistered** → run the [e-reporting recording workflow](/guides/fr-pa-reporting#recording-workflow) so the transaction is reported to the PPF on the next periodic submission.

This keeps the **Record document** step in the send workflow from rejecting non-regulated traffic.

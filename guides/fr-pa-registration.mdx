...
The `gov-fr.agreement.sign` step generates the mandate and waits while the customer completes signing via the entry API (all endpoints under `/apps/gov-fr/v1/entry/{silo_entry_id}/`). Identity verification is required on both paths.

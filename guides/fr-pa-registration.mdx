...
import RegisterWorkflow, { frPaRegisterWorkflow } from '/snippets/workflows/fr/fr-pa-register.mdx';
import UnregisterWorkflow, { frPaUnregisterWorkflow } from '/snippets/workflows/fr/fr-pa-unregister.mdx';
...
<Note>
A single SIREN registered in the Annuaire can have several directory entries, registered under the same or different Plateformes Agréées. Each entry has its own identifier, mapped to the Peppol in the Party inbox's `code`, in one of the forms `SIREN`, `SIREN_SIRET`, `SIREN_SIRET_suffix`, or `SIREN_suffix`.
</Note>

<Accordion title="Example of French supplier" defaultOpen="true">
    ...
    * The person in `people` is the company representative required to sign the *mandat de facturation* (billing mandate), including their `role`.
    * The `peppol` inbox uses scheme `0225` (French CTC electronic address). A single SIREN registered in the Annuaire can have several directory entries, registered under the same or different Plateformes Agréées. Each entry has its own identifier, mapped to the Peppol inbox's `code`, in one of the forms `SIREN`, `SIREN_SIRET`, `SIREN_SIRET_suffix`, or `SIREN_suffix`.
      * If no inbox is specified, the Register for Peppol step assigns one automatically, using scheme `0225` with the supplier's bare SIREN as the code, ignoring any SIRET present in the supplier's identities.
      * To register a specific directory entry instead, set the inbox explicitly, for example:
        ...
        "inboxes": [
            {
                "key": "peppol",
                "scheme": "0225",
                "code": "526018155_INVOPOP"
            }
        ],
        ```

        This registers the endpoint `526018155_INVOPOP` under the SIREN `526018155` in the Annuaire.

        Here is the full example of a Party
  <Supplier />
</Accordion>

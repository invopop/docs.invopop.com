---
title: "France PA — registration"
sidebarTitle: "Registration"
description: Register a party in the French Annuaire and on the Peppol network.
---

import FranceResources from '/snippets/tables/france-resources.mdx';
import RegisterWorkflow from '/snippets/workflows/fr/fr-pa-register.mdx';
import FAQ from '/snippets/faqs/fr/composers/guide-pa-supplier.mdx';

<Note>
  Registration is required for any French SIREN that will issue, receive, or report transactions through Invopop as its Plateforme Agréée. Registration is a prerequisite for all flows: invoicing, status, and reporting.
</Note>

Each party (a SIREN) is registered once. The workflow signs the contract, records the party in the Annuaire, opens a reporting register, and publishes the party on the Peppol network. After it completes, the party can both send and receive invoices and is ready to start recording transactions for reporting.

The Annuaire requires every entry to be receive-capable, so there is no send-only option as in other Peppol countries. Internally, the workflow extends the standard [Peppol registration](/guides/peppol) with a French Annuaire step and the reporting register.

## Routing identifiers

The Annuaire identifies recipients by an **electronic address**, not by SIREN alone. The routing identifier is carried on the registered party as an inbox with scheme `0225` (the FRCTC electronic address scheme), and its `code` is validated to start with the party's SIREN. A single SIREN can have several entries — typically one per inbox, where an inbox may correspond to an establishment, a department, or a different PA. Codes follow one of these patterns:

- `SIREN` — routes to the company as a whole.
- `SIREN_SIRET` — routes to a specific establishment within the company.
- `SIREN_SIRET_ROUTINGCODE` — routes to a specific target inside an establishment (e.g. a department or downstream system).
- `SIREN_SUFFIX` — a custom-suffixed inbox, used when a SIREN needs several addresses without distinct SIRETs.

By default the workflow registers the party under its `SIREN` identifier. When a sender looks the party up using only its VAT ID, the lookup step picks the simplest available identifier — `SIREN` first, then `SIREN_SIRET`, and so on — so the company-wide address is preferred whenever it exists.

## How it works

<Steps>
  <Step title="Sign Contract">
    Kicks off the contract signature flow with the party. The job pauses here until the party returns the signed agreement.
  </Step>
  <Step title="Set State → processing">
    Moves the silo entry into a visible "in progress" state so operator dashboards reflect that registration is underway.
  </Step>
  <Step title="Wait for Approval">
    Pauses until Invopop's KYC review of the signed contract clears.
  </Step>
  <Step title="Register in Annuaire">
    Submits the party's SIREN to the French national directory so other PAs can route invoices to it.
  </Step>
  <Step title="Register Reporter">
    Opens a reporting record keyed by SIREN and stores the cadence chosen during onboarding. See [reporting](/guides/fr-pa-reporting#registration) for the cadence options.
  </Step>
  <Step title="Register Peppol Party">
    Publishes the party in SMP/SML with the `france` document group, enabling Peppol-level routing for sending and receiving.
  </Step>
  <Step title="Set State → registered">
    Marks the party as fully onboarded.
  </Step>
</Steps>

## Workflow code

<Tabs>
  <Tab title="Template">
    <Card iconType="duotone" title="France PA register party workflow" icon="code-branch" href="https://console.invopop.com/redirect/workflows/new?template=fr-fr-pa-register" cta="Add to my workspace">
      Registers a party in the Annuaire, on the Peppol network, and as a reporter.
    </Card>
  </Tab>
  <Tab title="Code">
    Copy and paste into a new [Empty Party workflow](https://console.invopop.com/redirect/workflows/new?template=empty-party).
    <RegisterWorkflow />
  </Tab>
</Tabs>

## Unregistration

<Warning>
  Our unregistration workflow will be available soon
</Warning>

When a party stops trading or moves to another platform, run an unregistration workflow. The workflow steps remove the party from the Annuaire, Peppol and close out its reporting register.

<Steps>
  <Step title="Unregister in Annuaire">
    Removes the inbox from the French directory.
  </Step>
  <Step title="Disable Reporter">
    Closes the reporting register so no more periodic reports are generated for this SIREN.
  </Step>
  <Step title="Unregister in Peppol">
    disables the party on Peppol so no further invoices are routed to it.
  </Step>
</Steps>

<Note>
  Unregistering does not delete historical reports already submitted to the PPF.
</Note>

## FAQ

<FAQ />

More available in our [France FAQ](/faq/france) section

---

<FranceResources />

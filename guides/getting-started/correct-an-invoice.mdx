...
A credit note is sent to the buyer to let them know they've been credited a certain amount. This doesn't modify the original invoice but adjusts the total amount due. Credit notes are typically issued when there's an error in the invoice, when goods are returned or services are rejected, or when the goods delivered are damaged.

### Debit Note (Rarely Used)

A debit note can be understood in two ways.

On the one hand, it can refer to a financial document issued by the supplier to request additional funds related to a previously issued invoice (e.g., to account for omitted shipping costs). This use case is widely accepted in most countries and is the one supported by Invopop's invoicing workflows.

In contrast, some countries interpret a debit note as a financial document issued by the buyer, usually to formally acknowledge that they owe additional funds to the supplier, often due to pricing errors, contract terms, or self-billing arrangements. This buyer-issued interpretation is not universally accepted. 

GOBL focuses on the supplier-issued version, which is more common in standardized invoicing systems. If a buyer issues a debit note, it would more likely fall under order management workflows, which GOBL models as an Order document rather than a debit note.

Either way, a debit note is generally not a document that is used very often.

### Corrective Invoice (Country-Specific)

In some countries, such as Spain and Poland, local regulations require a corrective invoice (also known as a rectifying invoice), when certain key details on an invoice need to be changed.

A corrective invoice is a new document that references and legally cancels the original one. It's typically used when there are significant errors in fields such as the buyer's name, tax ID, or invoice date, and it fully replaces the original invoice for legal and accounting purposes.

It's worth noting that not all countries support corrective invoices. In some cases, GOBL will automatically map a correction to a credit or debit note format, depending on what the local regime allows. This behavior helps ensure compliance even when the source document is structured as a corrective invoice.
...
<Tabs>
  ...
  <Tab title="API">
    ...
    * **Auto-generate a credit note**: You reference an existing invoice, and Invopop builds the credit note for you.
    ...
    Unlike a standard invoice entry, a credit note must:
    - Set the `$schema` field so that it points to the correct schema (https://gobl.org/draft-0/bill/credit-note)
    - Set the `type` field accordingly (`"credit_note"` instead of `"standard"`).

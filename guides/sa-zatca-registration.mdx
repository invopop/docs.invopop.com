...
import RegisterWorkflow, { saRegisterWorkflow } from '/snippets/workflows/sa/sa-register.mdx';
import UnregisterWorkflow, { saUnregisterWorkflow } from '/snippets/workflows/sa/sa-unregister.mdx';
...
<Steps>
  <Step title="Company details">
    ...
    The supplier confirms the legal name and provides the **invoice type** (`1000` standard, `0100` simplified, or `1100` both), **branch name**, and **business category**. These values feed the certificate request, together with the registered address derived from the party’s national address.
  </Step>

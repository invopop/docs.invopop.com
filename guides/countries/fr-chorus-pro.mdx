...
<AccordionGroup>
  ...
  <Accordion title="ChorusPro registration KOs after 1 month with 'too many queued', what's the best way to handle this timeout?">
    If you wish to extend the registration window, we recommend adding [a webhook](https://docs.invopop.com/guides/features/webhooks) in the error handling section of your workflow. In the webhook response you can look through the `faults` array for a "too many queued" message and run the job again to extend the registration window.
  </Accordion>

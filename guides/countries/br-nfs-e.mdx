...
The NFS-e (_Nota Fiscal de Serviços Eletrônica_) is the electronic document that Brazilian service providers must issue to report service transactions to municipalities for ISS tax collection. Each municipality has its own rules, format, and API for issuing service invoices.
...
This guide will walk you through the steps required first to register a supplier and then issue service invoices in their name.

## Prerequisites

To issue service invoices in Brazil, you will need:
...
- A certificate and password accepted by the municipality to send service invoices in the name of every supplier.
...
In this section, we'll provide details on how to first persist and register a supplier, followed by issuing service invoices on their behalf.
...
In this example, we're issuing a simple service invoice from a Brazilian supplier to another Brazilian customer.

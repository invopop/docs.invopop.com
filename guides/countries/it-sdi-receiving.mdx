...
2. **Import Invoices Workflow** with `Import from SDI (Italy)` step. This workflow will convert received FatturaPA documents into our open source GOBL format. This workflow, as it handles invoices, must be of type `Invoices`.

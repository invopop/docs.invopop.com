...
<Tabs>
  <Tab title="Template">
    ...
    This workflow marks Suppliers as `Registered`. You will select this workflow in the PT-AT app configuration later.
    </Card>
  </Tab>
  <Tab title="Code">
  ...
  <Tab title="Build from scratch">
      ...
      In [Console](https://console.invopop.com), create a new workflow and choose **Empty Party workflow** as the base. Then name the workflow with a descriptive label such as "PT-AT Post-registration workflow".
...
Rename to "Post-registration workflow", and add a single step to change the state to `registered`.

The app will execute this workflow automatically after a supplier has successfully registered and is ready to issue documents. So, for instance, you can add a Webhook, or a Slack step at the end of this workflow to get a notification when that happens.
...
Enter the **Configuration** section, then click **Apps**. Click on the **Connect** button of the **AT Portugal** app.

Click the **Configure** button on the App, and choose the "Post-registration workflow" we created in the previous step, or leave empty if you don't want to use it.
...
Finally, we need to prepare the actual workflow that will process invoices and other documents. For this example, you can name the workflow something like "Issue AT document". It will consist of the following steps:
...
<Accordion title="Issue AT invoice workflow example">
Copy and paste the following example in the workflow editor:
<IssueInvoice/>
</Accordion>

...
<Steps>
  <Step title="Run the Supplier Registration Workflow">
    ...
    Then click **+New Supplier**:
      - *Live*: Enter the supplier's data manually or use developer mode (`</>` icon).
      - *Sandbox*: Switch to developer mode and paste the sample data below.
  ...
  <Step title="Complete Supplier Registration">
    ...
    To obtain these files:
      - *Live*: Follow this [guide](https://sw.com.mx/blog/administracion-fiscal/sellos-digitales-que-son-y-como-funcionan) to generate the CSD certificate and key.
      - *Sandbox*: Use the test `.crt`, `.key`, and private key password associated with your supplier's RFC, available on the [SW Sapien test page](https://developers.sw.com.mx/knowledge-base/donde-encuentro-csd-de-pruebas-vigentes/).

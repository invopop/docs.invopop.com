...
Once the invoice is sent, the AdE will automatically generate a document number for the invoice. Once the invoice is sent, you will find the document number in a stamp inside the document header.
...
After the AdE accepts the invoice, we add their reference to each line item. This reference facilitates the refund process. The example below shows how the `it-ticket-line-ref` extension appears in the accepted invoice.
...
This example shows a corrective invoice that refunds two units of "Cleaning Products" from the original invoice. Note that:

- The `it-ticket-line-ref` extension from the original invoice is preserved
- The `preceeding` object references the original invoice
- The corrective invoice maintains the same line item references for accurate tracking

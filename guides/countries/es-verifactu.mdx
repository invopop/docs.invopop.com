...
1. A signed and stamped [direct representation agreement](https://sede.agenciatributaria.gob.es/static_files/Sede/Procedimiento_ayuda/ZC01/Mod/Anexo_I_mod_otorg_prof_trib.pdf). If the supplier is an _autónomo_, stamping is unnecessary.
2. A copy of signatory's fiscal ID (DNI/NIE).

These documents will be collected through a multi-step web form (_wizard_) which 
...
<Steps>
  ...
  <Step title="Send the Supplier to the registration workflow">
    ...
    ***
    ...
    The supplier will receive an email with the registration link, and the supplier's status will change to `Processing` while waiting for supplier registration and verification. If these steps are completed successfully, the supplier's status will change to `Registered`. Otherwise, the status will change to `Error`, and you can inspect the job history of this document to see the reason.
...
<AccordionGroup>
  ...
  <Accordion title="What is the minimum information required to register a supplier?">
    ...
    If the entity is an _autónomo_, only the information requested in in the **Legal representative** section is required.

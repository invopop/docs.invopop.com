...
The process is best explained looking at the workflow steps:
    ...
    1. **Register with VERI\*FACTU** will generate a unique URL and add it into a `url` key value within the `meta` property.
    2. **Notify via Email:** sends an email to the supplier with the registration link. This step can be replaced with a webhook if you wish to handle notifications within your own systems. 
    3. **Set State to Processing** labels the silo entry so you know it's in process of registration.  
    4. **Wait for Supplier Registration VERI\*FACTU** waits for the supplier to upload the required documents (signed agreement and ID).
    5. **Wait for Approval VERI\*FACTU** Invopop will review the documents provided by the supplier. If there are any problems the workflow will error with a `KO` and the reason for rejection can be reviewed in the Supplier's job history.
    6. **Set State to Registered** labels the silo entry so you know registration is complete and approved.

...
<AccordionGroup>
  ...
  <Accordion title="What do I need to include in the field `notes` with key `general`?">
    The notes field with key `general` is mapped to the `DescripcionOperacion` field in Verifactu. This field is mandatory in the Verifactu schema, but the content itself is not validated—so you can include any text you'd like.

    If no value is provided, we automatically populate it using a concatenation of the line item names from the invoice.
  </Accordion>

...
<Steps>
  <Step title="Connect the VERI*FACTU app">
    ...
    After connecting, you'll see VERI\*FACTU listed in the Enabled Apps section. If your use case requires you to register third-party suppliers ([white labeling](/guides/use-cases/white-label)) we strongly recommend uploading a logo for your supplier registration portal. To do this:


    1. Click on the **Configure** button of the **VERI\*FACTU Spain** app you just enabled.
    2. In the **Logo for Supplier Registration Portal** field, and click on **Upload** and select an image from your local filesystem. We recommend a horizontal logo with at least 40px of height in PNG format (gif, png and jpg allowed).

    After completing this set-up, your supplier portal will display your brand, e.g.:

    <Frame>
      <img
        width="600"
        src="/guides/images/es-verifactu-register-supplier-logo.png"
        alt="VERI*FACTU Supplier Portal logo"
      />
    </Frame>
  ...
  <Step title="Configure the Supplier Registration Workflow">
    ...
    <Tabs>
      ...
      <Tab title="Build from scratch">
        ...
        1. **VERI\*FACTU Register Supplier** - Sends an email to the supplier with a registration link.
        2. **Set State** — select "Processing".
        3. **VERI\*FACTU wait for Registration** - Configure the wait time (in days) for the supplier to complete the registration form, sign agreements and attach the required documentation.
        4. **VERI\*FACTU wait for Validation** - Invopop will validate the provided supplier documentation in less than 72 hours. If rejected it will generate an error and block the supplier from issuing invoices. 
...
1. A signed [direct representation agreement](https://sede.agenciatributaria.gob.es/static_files/Sede/Procedimiento_ayuda/ZC01/Mod/Anexo_I_mod_otorg_prof_trib.pdf) where the entity allows Invopop to submit invoices to AEAT on its behalf.
2. A copy of signatory's fiscal ID (DNI/NIE)
3. Documentation proving that signatory is authorized to represent the entity, such as extract from commercial registry, company bylaws or power of attorney. `We are waiting on a response from the AEAT regarding this requirement` 

The collection of these documents is handled by the _Supplier Registration Workflow_ we created during setup. Let's see how.
...
<Steps>
  ...
  <Step title="Send the Supplier to the registration workflow">
    ...
    ***
    ...
    The supplier's status will change to `Processing` while waiting for supplier registration and verification. If these steps are completed successfully, the supplier's status will change to `Registered`. Otherwise, the status will change to `Error`, and you can inspect the job history of this document to see the reason.
...
<AccordionGroup>
  ...
  <Accordion title="What happens if a supplier does not complete their registration within the allotted wait time?">
    A KO will be triggered and the supplier will be labelled with the `Error` state. We currently recommend sending a  reminder to the supplier through a webhook, however, in the next few weeks we will automate this process from the workflow itself.
  ...
  <Accordion title="What happens if a supplier validation is rejected?">
    A KO will be triggered and the supplier will be labelled with the `Error` state. We currently recommend sending a  notification to the supplier through a webhook, however, in the next few weeks we will automate this process from the workflow itself.

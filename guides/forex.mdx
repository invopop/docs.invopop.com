...
## Overview
Many countries require that invoices issued in foreign currencies include an exchange rate to the national currency, and they even specify which source to use for the exchange rate. For example, when a Spanish company issues an invoice in USD, it must convert it to EUR, using the latest ECB rates, before reporting it to the Spanish tax authority.

By including this step in your workflows, you can ensure you will always use the correct exchange rate. 
...
<Accordion title="What happens if a currency in the exchange rates is not supported by the ECB?">
  If any of the currencies in the `exchange rates` field isn't supported by the ECB, the system will skip updating the exchange rate for that currency. No error will be thrown, and the process will continue.

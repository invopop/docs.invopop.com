...
<Accordion title="Italian company example">
  ...
  <ItCustomer />
  This is the same party shape used when you invoice a company, so the example carries an `it-sdi-code` inbox. Registration ignores it: it identifies the company fiscally, and once registered the company receives through Invopop's own recipient code, `BDCS3JD`.
</Accordion>

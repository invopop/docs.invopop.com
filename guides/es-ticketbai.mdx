...
- Supplier details, including:
  ...
  - address which includes the Region name, e.g. ("Alaba", "Bizkaia", or "Gipuzkoa").
...
<Tip>
  ...
  issued by the TicketBAI tax authorities.
</Tip>
...
Currently suppliers do not need to be registered with Invopop and we assume that a service contract exists either directly with the supplier, or via a third party that will allow Invopop to issue invoices on their behalf.

Legal contracts are not stored with suppliers inside Invopop at this time.

### Invoice Workflow

See the [workflows guide](/guides/workflows) for details on how to create a workflow.

To correctly issue TicketBAI invoices, your workflow will need to perform the following steps:

<Steps>
<Step title="Add Sequential Code">
TicketBAI requires all invoices to be assigned a series and code. Codes must be assigned sequentially without gaps in the numbering.
...
<Tip>
  Invoices must be signed before processing with TicketBAI. If you choose not to use the _Add Sequential Code_ step, be sure to add the _Sign Envelope_ step instead.
</Tip>
</Step>

<Step title="Generate TicketBAI (Spain)">
This will convert the GOBL document into a TicketBAI XML and perform the chaining steps in the background to ensure that every document is linked to a preivous one.

No further configuration is required. During the XML document generation, the GOBL document will be updated with the QR code required for the PDF generation in the next step.

</Step>

<Step title="Generate PDF">
  Invopop's PDF generator will automatically include the TicketBAI QR code
  generated in the previous step. If required, you could send a webhook
  notification to your own services after this step.
</Step>

<Step title="Send TicketBAI (Spain)">
In this step the workflow will send the document to one of the three TicketBAI endpoints for processing.

<Warning>
If in the unlikely scenario a document is rejected in this step, you will need to try and correct the error inside a new document and resend. As documents are all chained together in the _Generate TicketBAI_ step, modifications are not permitted.
</Warning>
</Step>

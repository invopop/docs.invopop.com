...
import WorkflowExample from '/snippets/workflows/templates/es/sii-invoice.mdx';
...
import CancelInvoice from '/snippets/workflows/templates/es/sii-cancel.mdx';
import SpainResources from '/snippets/tables/spain-resources.mdx';

## Introduction

SII (Suministro Inmediato de Información) is a fiscal initiative by the Spanish Tax Agency (AEAT) that requires large businesses to report invoices through certified software. The system submits invoices in the SII format to the tax authorities.

<Note>SII is mandatory for companies that meet certain requirements. Companies reporting with VERI*FACTU or TicketBAI are exempt from reporting with SII.</Note>

## Prerequisites

To issue your first SII invoice, you must have had registered at least one supplier. Make sure you read and implemented the [SII supplier onboarding guide](/guides/es-sii-supplier) before continuing with this guide.
...
<Tabs>
  <Tab title="Template">
    <Card title="SII invoice workflow" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-sii-invoice" cta="Add to my workspace">
      This workflow will issue an invoice through SII.
    </Card>
  </Tab>
  <Tab title="Code">
  ...
  <Tab title="Build from scratch">
    ...
    1. **Sign Envelope** - Signs the SII document.
    3. **Record invoice in SII** - Records the invoice in SII.
...
<Steps>
  <Step title="Upload an Invoice">
    ...
    <AccordionGroup>
      <Accordion title="B2C Standard Invoice">
        ...
        To create a simplified SII invoice, include the `simplified` tag in your GOBL document with the [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon. The addon will automatically set the [document type](https://docs.gobl.org/addons/es-sii-v1#invoice-type) to `F2` (Simplified invoice) when the silo entry is built.
      ...
      <Accordion title="B2B Standard Invoice">
        ...
        To create a regular SII invoice, include the [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon and ensure the `customer` field contains valid tax identification details. The addon will automatically set the [document type](https://docs.gobl.org/addons/es-sii-v1#invoice-type) to `F1` when the silo entry is built (which happens automatically when saving a silo entry through the API or console).
      ...
      <Accordion title="B2B Services EU Client (Reverse Charge)">
        ...
        To create a reverse charge SII invoice, use the tax key `reverse-charge` in your invoice lines. This will automatically set the operation type to `S2` (Subject and Not Exempt - With reverse charge) when reporting the document to SII.

        You also need to set the items `key` to `services`. This will generate a breakdown by operation type which is required for this type of invoices.
      ...
      <Accordion title="B2B Goods EU Client (Intra-Community)">
        ...
        To create an intra-community supply SII invoice, use the tax key `intra-community` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the [exemption reason](https://docs.gobl.org/addons/es-sii-v1#exemption-reason) to `E5` ([Artículo 25 Ley de IVA](https://www.boe.es/eli/es/l/1992/12/28/37/con#a25)) and the [special regime code](https://docs.gobl.org/addons/es-sii-v1#special-regime-or-relevance-key) to `02` (Export) when the silo entry is built.

        You also need to set the items `key` to `goods`. This will report to SII a breakdown by operation type which is required for this type of invoices.
      ...
      <Accordion title="B2B Services No EU Client (Outside Scope)">
        ...
        To create an outside scope SII invoice, use the tax key `outside-scope` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the [type of operation outside scope](https://docs.gobl.org/addons/es-sii-v1#type-of-operation-outside-scope) to `location` when the silo entry is built.
      ...
      <Accordion title="B2B Goods No EU Client (Export)">
        ...
        To create an export SII invoice, use the tax key `export` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the [exemption reason](https://docs.gobl.org/addons/es-sii-v1#exemption-reason) to `E2` ([Artículo 21 Ley de IVA](https://www.boe.es/eli/es/l/1992/12/28/37/con#a21)) and the [special regime code](https://docs.gobl.org/addons/es-sii-v1#special-regime-or-relevance-key) to `02` (Export) when the silo entry is built.
      ...
      <Accordion title="B2B Exempt E1 Invoice">
        ...
        To create an exempt E1 SII invoice, use the tax key `exempt` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the exemption code to `E1` when the silo entry is built.
      ...
      <Accordion title="B2C One-Stop-Shop Invoice">
        ...
        To create an OSS SII invoice, include the `simplified` tag in your GOBL document with the [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon. The invoice will be treated as a simplified invoice ([document type](https://docs.gobl.org/addons/es-sii-v1#invoice-type) `F2`) and VAT is accounted for through the OSS scheme in the destination country.
  ...
  <Step title="Send the Invoice to the SII invoice workflow">
    ...
    - `workflow_id`: use the Workflow ID of the _SII invoice_ workflow created on setup.
...
<Steps>
  ...
  <Step title="Send the Credit Note to the SII invoice workflow">
    ...
    - `workflow_id`: use the Workflow ID of the _SII invoice _workflow we created during setup
...
<Steps>
  ...
  <Step title="Send the Corrective Invoice to the SII invoice workflow">
    ...
    - `workflow_id`: use the Workflow ID of the _SII invoice_ workflow we created during setup

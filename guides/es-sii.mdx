...
To issue your first SII invoice, you must have had registered at least one supplier. Make sure you read and implemented the [SII supplier registration guide](/guides/es-sii-supplier) before continuing with this guide.
...
<Steps>
  <Step title="Upload an Invoice">
    ...
    <AccordionGroup>
      ...
      {/*<Accordion title="B2B Services EU Client (Reverse Charge)">
        ...
        To create a reverse charge SII invoice, use the tax key `reverse-charge` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the operation class to `S2` (Subject and Not Exempt - With reverse charge) when the silo entry is built.
      ...
      <Accordion title="B2B Goods EU Client (Intra-Community)">
        ...
        To create an intra-community supply SII invoice, use the tax key `intra-community` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the exemption code to `E5` ([Artículo 25 Ley de IVA](https://www.boe.es/eli/es/l/1992/12/28/37/con#a25)) when the silo entry is built.
      ...
      <Accordion title="B2B Services No EU Client (Outside Scope)">
        ...
        To create an outside scope SII invoice, use the tax key `outside-scope` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the operation class to `N2` (Not Subject - Due to location rules) when the silo entry is built.
      ...
      <Accordion title="B2B Goods No EU Client (Export)">
        ...
        To create an export SII invoice, use the tax key `export` in your invoice lines. The [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon will automatically set the exemption code to `E2` ([Artículo 21 Ley de IVA](https://www.boe.es/eli/es/l/1992/12/28/37/con#a21)) and regime code to `02` (Export) when the silo entry is built.
      ...
      <Accordion title="B2C One-Stop-Shop Invoice">
        ...
        To create an OSS SII invoice, include the `simplified` tag in your GOBL document with the [`es-sii-v1`](https://docs.gobl.org/addons/es-sii-v1) addon. The invoice will be treated as a simplified invoice (document type `F2`) and VAT is accounted for through the OSS scheme in the destination country.
  ...
  <Step title="Send the Invoice to the SII invoice workflow">
    ...
    {/*
    <Frame caption="Example SII invoice with QR">
      <img
        width="420"
        src="/assets/guides/es-sii-example-pdf.png"
        alt="Example SII invoice with QR"
      />
    </Frame>
    */}
  </Step>
...
<Steps>
  ...
  After successful upload, you'll see the corrective invoice in the **Invoices** section with status `Empty`. If you've included the `uuid` of the invoice in preceding, you'll see a link to the corrective invoice we've just created in the _Related_ tab of the invoice.
    ...
    <Frame caption="Link to the corrective invoice">
      <img
        width="600"
        src="/assets/guides/es-sii-invoice-corrective.png"

---
title: "VERI*FACTU onboarding guide"
sidebarTitle: "Supplier registration"
description: "Create and register suppliers in Spain with VERI*FACTU."
---

import UnregisterExample, { esVerifactuUnregisterWorkflow } from '/snippets/workflows/es/verifactu-unregister.mdx';
import SupplierRegistration, { esVerifactuSupplierRegistrationWorkflow } from '/snippets/workflows/es/verifactu-supplier-registration.mdx';
import SupplierAccordion from '/snippets/parties/es/accordion.mdx';
import SpainResources from '/snippets/tables/spain-resources.mdx';
import FAQ from '/snippets/faqs/es/composers/guide-verifactu-supplier.mdx';
...
<Steps>
 <Step title="Connect the VERI*FACTU app">
  ...
  <Step title="Create a supplier registration workflow">
    ...
    <Card title="VERI*FACTU supplier registration workflow" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-verifactu-supplier-registration" horizontal>
      Add to my workspace →
    </Card>
    <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={esVerifactuSupplierRegistrationWorkflow} />
      </Tab>
      <Tab title="Code">
    ...
    <Tabs>
      <Tab title="Your own company">
        ...
        <Frame>
          <img
            width="398"
            src="/assets/guides/es-verifactu-entry-meta.png"
...
These documents can be collected through a multi-step web form (_wizard_) which is enabled through the _Supplier registration workflow_ created during [setup](#setup) or [sent via the API](/api-ref/apps/verifactu/agreement-upload) if you wish to have control over the process.
...
Unregistering a supplier is essentially revoking their invoice issuing privileges. You might need to unregister an entity because the supplier is no longer your client or because of a change in their Tax ID. To do so, use the following workflow template:
  ...
  <Card title="VERI*FACTU unregister supplier" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-verifactu-unregister" horizontal>
    Add to my workspace →
  </Card>
  <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={esVerifactuUnregisterWorkflow} />
      </Tab>
      <Tab title="Code">
        Copy and paste into a new [Empty Party workflow](https://console.invopop.com/redirect/workflows/new?template=empty-party) code view.

...
<Warning>
  This guide is currently a work in progress as the we stive to make the setup
...
4. Add a first step using the “Series Enumerator” provider. The simplest setup here would be to use a "Dynamic" series. That will allow you to use the same workflow for both invoices and credit notes. Make sure to configure it according to the parameters of the authorised numbering range.

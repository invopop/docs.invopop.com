...
import PartyRegistrationSendWorkflow, { plRegisterWorkflow } from '/snippets/workflows/pl/pl-register.mdx';
...
In this guide you'll learn how to register a party using the [Poland app](/apps/poland). Once a party is registered, continue with the companion guides: [Poland: Issuing invoices](/guides/pl-ksef) and [Poland: Receiving invoices](/guides/pl-ksef-receiving).
...
<Info>
  There is also a **send & receive** variant of this workflow that additionally subscribes the party to periodic invoice imports via the [Cron app](/apps/cron). It is covered in the [receiving invoices guide](/guides/pl-ksef-receiving).
</Info>
...
Generate a random Polish Tax ID (NIP) at [http://generatory.it/](http://generatory.it/) and use it in place of `YOUR_RANDOM_NIP`:
...
```json
{
  ...
  "tax_id": {
    "country": "PL",
    "code": "YOUR_RANDOM_NIP"
  }
}
```

<Warning>
  KSeF's test environment is shared by everyone integrating with it, and anyone can send invoices to any NIP. Avoid NIPs from examples or other easy-to-guess values: your party would also receive other testers' invoices, and receiving could exceed KSeF's download limits.
</Warning>

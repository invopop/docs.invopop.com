...
- **POST** `/silo/v1/entries/{entry_id}/files` <br />
System generates the UUID.
- **PUT** `/silo/v1/entries/{entry_id}/files/{id}` <br />
...
Use the `url` field from the response to upload the file contents:
...
View the response fields for the file upload endpoints in our [File Upload API Reference](/api-ref/silo/entries/files-post#response-category).
...
- [Create a File (POST)](/api-ref/silo/entries/files-post) — API reference for POST endpoint
- [Create a File (PUT)](/api-ref/silo/entries/files-put) — API reference for PUT endpoint
- [Fetch a File](/api-ref/silo/entries/files-fetch) — Retrieve uploaded files

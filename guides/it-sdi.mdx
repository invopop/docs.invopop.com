...
This step will also return a code that provides more information about the response from SDI. The codes can be one of the following:

| Code | Description |
|------|-------------|
| `rejected` | **Notifica di scarto (NS)** - SDI has rejected your invoice because it contains errors. |
| `delivered` | **Ricevuta consegna (RC)** - SDI has successfully delivered your B2B invoice to the recipient. |
| `delivered-pa` | **Ricevuta consegna (RC)** - SDI has successfully delivered your invoice to a Public Administration (PA). |
| `not-delivered` | **Mancata consegna (MC)** - SDI could not deliver the invoice to the recipient. |
| `not-delivered` | **Attestazione trasmissione con impossibilità di recapito (AT)** - SDI couldn't deliver your invoice to PA after 10 days. |
| `accepted-pa` | **Notifica esito (NE)** - A PA has decided to accept your invoice (outcome EC01). |
| `rejected-pa` | **Notifica esito (NE)** - A PA has decided to reject your invoice (outcome EC02). |

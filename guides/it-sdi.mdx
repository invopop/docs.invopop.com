...
The initial connection requires no configuration. However, additional setup will be needed later if you plan to receive invoices.
...
### Configure app for receiving invoices
...
To receive invoices through SDI, complete these three important steps for each company:

1. **Register with Italian tax authority** - Register the Recipient Code `JKKZDGR` with the Italian tax authority for each company that needs to receive invoices, as outlined in the setup steps.

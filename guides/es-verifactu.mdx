...
1. `Sign Envelope`
   - Signs the VeriFactu document.
   - This step can be substituted with the `Add Sequential Code` step (with the `Sign Document` option enabled), which also handles the invoice number.

2. `Generate VeriFactu (Spain)`
   - Creates the required XML document. It can be seen in the `Files` section of the console.
   - Adds the document to the chain.
   - Adds the required fingerprint to the document.

3. `Send VeriFactu (Spain)`
   - Submits the generated XML to AEAT gateway and returns a response.

4. `Generate PDF`
   - Generates the PDF invoice with the required QR code.
   - Supports additional configuration.
...
VeriFactu is currently scheduled to be mandatory for businesses in Spain from July 1st, 2025. However, it is likely that the deadline will be extended to January 1st, 2026.
...
1. The recommended approach is to issue a credit note, which in VeriFactu translates to a `Factura Rectificativa por Diferencias`.
2. If the above approach does not apply, you can issue a debit note, which in VeriFactu translates to a `Factura Rectificativa por Sustitución`.

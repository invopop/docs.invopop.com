...
import WorkflowExample, { esVerifactuInvoiceWorkflow } from '/snippets/workflows/es/verifactu-invoice.mdx';
...
import CancelInvoice, { esVerifactuCancelWorkflow } from '/snippets/workflows/es/verifactu-cancel.mdx';
import SpainResources from '/snippets/tables/spain-resources.mdx';
import FAQ from '/snippets/faqs/es/composers/guide-verifactu-invoicing.mdx';
...
<Note>VERI\*FACTU will be mandatory for companies from January 1st, 2027. Remaining taxpayers must comply from 1 July 2027. Companies reporting with SII or TicketBAI are exempt from reporting with VERI\*FACTU.</Note>

## Prerequisites

To issue your first VERI\*FACTU invoice, you must have registered at least one supplier. Make sure you read and implemented the [VERI\*FACTU supplier registration guide](/guides/es-verifactu-supplier) before continuing with this guide.
...
<Card title="VERI*FACTU invoice workflow" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-verifactu-invoice" horizontal>
  Add to my workspace →
</Card>
<Tabs>
  <Tab title="Workflow">
    <WorkflowDiagram workflow={esVerifactuInvoiceWorkflow} />
  </Tab>
  <Tab title="Code">
...
<Prompt description="Have your agent issue a VERI*FACTU invoice in your sandbox." icon="file-invoice" actions={["copy", "cursor"]}>
Read https://docs.invopop.com/guides/es-verifactu.md, https://docs.invopop.com/guides/es-verifactu-supplier.md and https://docs.gobl.org/addons/es-verifactu-v1.md. In my sandbox workspace, using the token in INVOPOP_TOKEN and the workflow ID I give you, create a bill/invoice entry with $regime ES, $addons containing es-verifactu-v1, the pre-enabled sandbox supplier from the guide, and the lines I describe with taxes as the VAT category plus a rate key. Do not write totals or percentages and do not invent extension codes; start from one of the example invoices in the guide. Run the workflow with wait=30, then fetch the entry and report its state, the stamps in the envelope header, the attached XML and PDF, and any faults with their AEAT error codes explained using https://docs.invopop.com/faq/spain.md.
</Prompt>
...
<Steps>
  <Step title="Add a cancel invoice workflow (only once)">
    ...
    <Card title="VERI*FACTU cancel invoice workflow" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-verifactu-cancel" horizontal>
      Add to my workspace →
    </Card>
    <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={esVerifactuCancelWorkflow} />
      </Tab>
      <Tab title="Code">
        Copy and paste into a new [Empty Invoice workflow](https://console.invopop.com/redirect/workflows/new?template=empty-invoice) code view.

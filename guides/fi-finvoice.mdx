...
description: Issue invoices to Finnish receivers on the domestic operator network.
---

import SendInvoiceWorkflow, { fiFinvoiceSendInvoiceWorkflow } from '/snippets/workflows/fi/finvoice-send-invoice.mdx';
...
Finnish businesses receive their e-invoices over two networks: the domestic operator network (*Finvoice*/*TEAPPSXML*) and Peppol. The [Finland app](/apps/finland) delivers on the domestic network, through Invopop's partner operator, Apix Messaging. You send a GOBL invoice with the receiver's e-invoice address, and the receiver gets it in the format its operator expects.

If your receiver is only on Peppol, use the [Peppol app](/apps/peppol). To find out which network a receiver is on, look them up at [verkkolaskuosoite.fi](https://verkkolaskuosoite.fi), which lists both.

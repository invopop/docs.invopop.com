...
Invopop has partnered with [eCourier](https://ecourier.dk) as its NemHandel access point. The Denmark app converts [GOBL](https://docs.gobl.org) invoices and credit notes to OIOUBL, delivers them over the network, and imports documents your suppliers receive. For onboarding suppliers, see the companion guide: [Denmark: Supplier registration](/guides/dk-nemhandel-supplier).

| - | Sandbox | Live |
|---|---|---|
| **Supplier** | Registered against the eCourier sandbox | Registered supplier required |
...
- **A registered supplier**: follow the [NemHandel supplier registration guide](/guides/dk-nemhandel-supplier) to connect the Denmark app, sign the authorization agreement, and register the supplier on the network before issuing.
...
<Steps>
  <Step title="Create the send workflow">
    This workflow numbers and signs the invoice, converts it to OIOUBL 2.1, and submits it to the network, holding until eCourier confirms delivery.
  ...
  <Step title="Configure the Denmark app">
    <Info>You can skip this step if you don't plan to receive invoices.</Info>

    In **Configuration** → **Apps** → **Denmark**, select the import workflow you just created as the **incoming workflow**. Every document received on the network for one of your registered suppliers is handed to it automatically. See the [Denmark app](/apps/denmark) page for the full list of actions it provides.
  </Step>
</Steps>
...
The workflow queues after submission until eCourier confirms the delivery, so an entry in the `sent` state means the recipient's access point accepted the document.

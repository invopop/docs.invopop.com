...
<Prompt description="Have your agent wire the token into this project and verify it." icon="key" actions={["copy", "cursor"]}>
Read https://docs.invopop.com/guides/authentication.md. Add Invopop API access to this project: read the token from the INVOPOP_TOKEN environment variable or this project's secrets mechanism, never from source code, and send it as an Authorization Bearer header to https://api.invopop.com. Implement a small client with a ping method that calls GET /utils/v1/ping and returns true on pong, plus a helper that fetches a UUID from GET /utils/v1/uuid?v=7 for idempotent PUT requests. Keep one client instance per workspace, because each Invopop workspace has its own token. Add a test that calls ping when the variable is set and is skipped otherwise.
</Prompt>

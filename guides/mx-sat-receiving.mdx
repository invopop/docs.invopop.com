...
import Contact from '/snippets/suppliers/mx/sat-supplier.mdx';
...
<Steps>
  <Step title="Create the party">
    In the left sidebar, go to the **Parties** section and click **Contacts**.

    Click **+New Contact** and enter the party's data manually using the console, or switch to developer mode (`</>` icon) to paste the data directly. At a minimum, provide:

    - **Name** of the company
    - **Tax ID Country** with `MX`
    - **Tax ID Code** with the entity's RFC (_Registro Federal de Contribuyentes_)

    Here is an example contact for reference:

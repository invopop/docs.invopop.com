...
<Tabs>
  ...
  <Tab title="Code">
    ...
    After saving, open the **Subscribe to Periodic Import** step and set the **Workflow** field to the bulk import CFDI workflow created in the previous step.
  </Tab>
  <Tab title="Build from scratch">
    Before starting, review the [workflows guide](/guides/features/workflows) to understand the general setup process.
...
<Steps>
  <Step title="Create the party">
    ...
    Click **+New Supplier** and enter the party's data manually using the console, or switch to developer mode (`</>` icon) to paste the data directly. At a minimum, provide:

...
- `transform_job_id`  the id of a job run through the [Transform Job endpoint](/api-ref/transform/jobs/fetch-a-job-by-key)
- `silo_entry_id` the id of the [silo entry](/api-ref/silo/entries/fetch-an-entry)
- `key` the job's [key](/api-ref/transform/jobs/create-a-job-put#body-key) to ensure [idempotency](/api-ref/idempotency).
...
<Prompt description="Have your agent build a webhook receiver for job results." icon="webhook" actions={["copy", "cursor"]}>
Read https://docs.invopop.com/guides/webhooks.md and https://docs.invopop.com/llms.md. Implement an HTTPS endpoint in this project that receives Invopop's Send Webhook POST requests. Verify the bearer token or custom header I configure in the workflow step. Parse the JSON body fields id, event, transform_job_id, silo_entry_id, key, args and faults, and treat deliveries as idempotent using id and key. Respond 200 quickly and process asynchronously. On success events fetch the silo entry from the Invopop API and store its state and attachment URLs. On error events store the faults from the payload and fetch the job for the per-step trace. Return a non-200 status only when Invopop should retry, and document the retry schedule from the guide in a code comment.
</Prompt>
...
If you wish to change this behavior, include a `retry_in` field in your response with the number of seconds you wish to wait between retries. The system will then retry up to 32 times before returning a `KO` status. Here is an example of such a response:

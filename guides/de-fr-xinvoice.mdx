...
## FAQ
<AccordionGroup>
<Accordion title="Which X-Rechnungsyntax should I choose, UBL or CII?">
Both syntaxes are equally valid and contain the same information, so in the majority of cases you can choose either one.

If you plan on sending Invoices through the PEPPOL network, we recommend using UBL as it is the syntax supported by it. In any other case, it might be more convenient to use CII, as it is the syntax also supported by ZUGFeRD and Factur-X.
</Accordion>

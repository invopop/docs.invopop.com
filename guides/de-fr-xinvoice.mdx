...
<Tip>
    ...
    In the context of E-Invoicing in the EU, the core semantic data model is established by the [EN16931 standard](https://ec.europa.eu/digital-building-blocks/sites/display/DIGITAL/EN+16931+compliance), which is the basis for the aforementioned standards. It defines a list of two approved syntaxes: UN/CEFACT Cross Industry Invoice
    ([CII](https://unece.org/trade/documents/2023/10/executive-guide-einvoicing-cross-industry-invoice)) version 16B and OpenOasis Universal Business Language ([UBL](https://docs.oasis-open.org/ubl/UBL-2.1.html)) version 2.1. These are the syntaxes used by this integration.
</Tip>
...
Be sure to set the `regime` field to `DE` or `FR` in your invoices. This will add some extra validations to ensure the document is compliant. If you plan to issue X-Rechnung invoices, we also recommend using the X-Rechnung addon in GOBL, which adds further validation.

...
<Warning>
  Every invoice run through the **Record Invoice** step (`gov-fr.directory.record`) must carry the `fr-ctc-flow2-v1` add-on. The action rejects invoices without it up front with a validation error, since the addon's normalization and validation rules are what make the envelope PPF-compliant. If you build a custom send or receive workflow, make sure the addon is applied to the GOBL invoice before it reaches this step — otherwise the invoice fails immediately instead of getting stuck later at PPF business validation. The example invoices on this page already include it.
</Warning>

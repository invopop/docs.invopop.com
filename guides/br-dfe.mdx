...
The following examples are of example [GOBL](https://docs.gobl.org) documents you can copy and paste directly into the [Invopop Console](https://console.invopop.com) or store via the API as silo entries. Then, you must run the "Issue NFS-e" or the "Issue NFC-e" workflow created during setup over them.
...
If you need to cancel a document that has already been sent to the tax authority, you can use the cancel workflows created during setup. Run the appropriate cancel workflow — NFS-e or NFC-e — over the silo entry you want to cancel. On success, the entry will be set to `Void`.
...
| Parameter | Step config field | Job argument | Applies to | Notes |
|---|---|---|---|---|
| Cancellation reason | `reason` | `nfe-br-cancel-reason` | NFS-e and NFC-e | Free-text reason for the cancellation. For NFC-e, must be at least 15 characters if provided. Defaults to the system default when omitted. |
| Cancellation code | `code` | `nfe-br-cancel-code` | NFS-e only | Municipality-specific cancellation code, for example `9` for "Outros" in the National NFS-e system. Defaults to the system default when omitted. |

    ...
    <Tabs>
      <Tab title="Your own company">
        ...
        <Frame>
          <img
            width="398"
            src="/assets/console/entry-meta-sii.png"
            alt="Supplier entry meta"
          />
        </Frame>
      </Tab>
      <Tab title="Third-party companies">
        Add a webhook before the **Wait for supplier registration** step to obtain the registration link. The `meta` object will look like this:
...
These documents can be collected through a multi-step web form (_wizard_) which enabled through the _Supplier registration workflow_ created during [setup](#set-up) or [sent via the API](https://docs.invopop.com/api-ref/apps/sii/agreement-upload) if you wish to have control over the process.
...
If you have special needs regarding the onboarding of suppliers for SII (such as using third party identity verification services), it's possible to implement your own onboarding process through the API. The process would be the following:
  ...
  2. [Create a job](/api-ref/transform/jobs/create-a-job-put)`https://api.invopop.com/transform/v1/jobs/{id}`<br />
    Using the supplier registration workflow ID and Silo Entry ID.

  3. [Generate agreement PDF](/api-ref/apps/sii/agreement-fetch) `apps/sii/v1/entry/:silo_entry_id/agreement`<br />
    The agreement file will be created with the supplier's data.

  4. [Upload agreement PDF](/api-ref/apps/sii/agreement-upload) `apps/sii/v1/entry/:silo_entry_id/agreement`<br />
    Upload the agreement into the supplier's silo entry.

  5. [Upload identity images](/api-ref/apps/sii/identity-upload) `apps/sii/v1/entry/:silo_entry_id/identity`<br />
    Upload the ID into the supplier's silo entry (can be DNI, NIE or Passport).
    
  6. [Confirm Upload](/api-ref/apps/sii/confirm) `https://api.invopop.com/apps/sii/v1/entry/:silo_entry_id/confirm`<br />

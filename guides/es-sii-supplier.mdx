---
title: "SII onboarding guide"
sidebarTitle: "Supplier registration"
description: "Create and register suppliers in Spain with SII."
---

import UnregisterExample, { esSiiUnregisterWorkflow } from '/snippets/workflows/es/sii-unregister.mdx';
import SupplierRegistration, { esSiiRegisterWorkflow } from '/snippets/workflows/es/sii-register.mdx';
import Supplier from '/snippets/parties/es/supplier.mdx';
import SpainResources from '/snippets/tables/spain-resources.mdx';
import FAQ from '/snippets/faqs/es/composers/guide-sii-supplier.mdx';
...
1. Walk you through the process of creating the workflows necessary to onboard yourself and/or your suppliers (in a [white-label setup](/guides/white-label)),
2. Explain how to complete the supplier onboarding wizard, and
3. Explain the approval process and what to do in case of rejection.



| - | Sandbox | Live |
|---|---|---|
| **Supplier registration** | Required (automated acceptance/rejection) | Required (documents verified automatically on submission) |
...
<Steps>
 <Step title="Connect the Spain app">
    ...
    After connecting, you'll see Spain listed in the Enabled Apps section.
  </Step>
  <Step title="Create a supplier registration workflow">
    Create a new workflow in your workspace for registering suppliers. Start with the template below:

    <Card title="SII supplier registration workflow" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-sii-register" horizontal>
      Add to my workspace →
    </Card>
    <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={esSiiRegisterWorkflow} />
      </Tab>
      <Tab title="Code">
...
Whether you issue invoices on behalf of third parties ([white label](/guides/white-label)) or under your own name, the process is essentially the same.
  ...
  <Steps>
    ...
    <Frame caption="Upload supplier to Invopop">
      <img
        width="600"
        src="/assets/guides/es-sii-supplier-2.png"
   ...
   <Step title="Complete the registration workflow">
    ...
    <Tabs>
      <Tab title="Your own company">
        Within the silo entry in console, select **Meta** and then click **sii.invopop.com**. This will launch the registration wizard for your company.

        <Frame>
          <img
            width="398"
            src="/assets/guides/es-sii-entry-meta.png"
        ...
        "meta": [
          {
            ...
            "key": "sii-link",
            "ref": "Kb01VKCpbxN",
            "link_url": "https://gov-es.invopop.com/reg/sii/Kb01VKCpbxN",
            "link_scope": "public",
            "indexed": true,
            "shared": true
          }
  ...
  <Step title="Check the registration result">

    Approval is automated: Invopop verifies the signed agreement and the signatory's ID as soon as the supplier submits them, so there is no manual review or waiting period. The supplier is entitled to issue invoices immediately, and that entitlement is only revoked if the verification finds a problem with their documentation.

    Once the registration wizard has been completed, the supplier's status will change to `Registered`. If not successful because the supplier did not complete the process or their documentation was rejected, the status will change to `Rejected`, and you can inspect the job history of this document to see the reason.

        <Frame caption="Supplier registered successfully">
          <img
            width="600"
            src="/assets/guides/es-sii-supplier-1.png"
  ...
  <Warning>If a supplier's agreement documentation is rejected and you need to onboard them again, unregister the supplier and run the registration workflow again (see [Unregister a supplier](#unregister-a-supplier)).</Warning>
...
The process is best explained looking at the workflow steps:
    ...
    <Frame caption="Supplier Registration Workflow">
      <img
        width="376"
        src="/assets/guides/es-sii-register-supplier.png"
    ...
    5. **Wait for supplier approval** In live environments, Invopop automatically verifies the documents provided by the supplier as soon as they are uploaded, so this step completes immediately. In sandbox, you can configure the workflow step to accept or reject providers. Upon rejection, the workflow will error with a `KO` and the reason for rejection can be reviewed in the Supplier or in the executed job. The supplier's invoice issuing entitlement will be revoked immediately.
...
1. A signed [direct representation agreement](https://sede.agenciatributaria.gob.es/static_files/Sede/Procedimiento_ayuda/ZC01/Mod/Anexo_I_mod_otorg_prof_trib.pdf).
...
These documents can be collected through a multi-step web form (_wizard_) which is enabled through the _Supplier registration workflow_ created during [setup](#setup) or [sent via the API](/api-ref/apps/gov-es/agreement-upload) if you wish to have control over the process.

### Onboard suppliers through the supplier registration wizard

The supplier registration wizard walks you (or your customers) through the necessary legal requirements to issue SII invoices. Each issuer must submit a signed representation agreement between Invopop and the issuer, along with documentation proving their legal authority to enter into such an agreement.
...
<Steps>
  <Step title="Welcome screen">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-1.png"
  ...
  <Step title="Information verification">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-2.png"
        alt="Supplier registration wizard"
      />
    </Frame>
  ...
  <Step title="Download agreement">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-3.png"
        alt="Supplier registration wizard"
      />
    </Frame>
  ...
  <Step title="Upload signed agreement">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-4.png"
        alt="Supplier registration wizard"
      />
    </Frame>
  ...
  <Step title="Upload ID">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-5.png"
        alt="Supplier registration wizard"
      />
    </Frame>
    The legal representative or _autónomo_ uploads a scan or clear photo of their ID in a valid image format (JPG or PNG). The ID should ideally be a DNI/NIE, as passports are only used when the person is not entitled to have one.
  ...
  <Step title="Review details">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-6.png"
        alt="Supplier registration wizard"
      />
    </Frame>
  ...
  <Step title="Confirmation screen">
    <Frame>
      <img
        width="400"
        src="/assets/guides/es-sii-supplier-wizard-7.png"
        alt="Supplier registration wizard"
      />
    </Frame>
    A green checkmark will indicate success. The submitted documents are verified automatically, so the supplier is approved and registered right away unless a problem is found.
  </Step>
...
If you have special needs regarding the onboarding of suppliers for SII (such as using third party identity verification services), it's possible to implement your own onboarding process through the API. The process would be the following:

  1. [Create a supplier entry](/api-ref/silo/entries/create-an-entry-put) `/silo/v1/entries/{id}`<br />
  ...
  3. [Generate agreement PDF](/api-ref/apps/gov-es/agreement-fetch) `/apps/gov-es/v1/entry/{silo_entry_id}/sii/agreement`<br />
    The agreement file will be created with the supplier's data.

  4. [Upload agreement PDF](/api-ref/apps/gov-es/agreement-upload) `/apps/gov-es/v1/entry/{silo_entry_id}/sii/agreement`<br />
    Upload the agreement into the supplier's silo entry.

  5. [Upload identity images](/api-ref/apps/gov-es/identity-upload) `/apps/gov-es/v1/entry/{silo_entry_id}/sii/identity`<br />
    Upload the ID into the supplier's silo entry (can be DNI, NIE or Passport).

  6. [Confirm upload](/api-ref/apps/gov-es/confirm) `/apps/gov-es/v1/entry/{silo_entry_id}/sii/confirm`<br />
...
Unregistering a supplier is essentially revoking their invoice issuing privileges. You might need to unregister an entity because the supplier is no longer your client, because their agreement documentation was rejected, or because of a change in their Tax ID. To do so, use the following workflow template:
  ...
  <Card title="SII unregister supplier" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=es-sii-unregister" horizontal>
    Add to my workspace →
  </Card>
  <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={esSiiUnregisterWorkflow} />
      </Tab>
      <Tab title="Code">
        Copy and paste into a new [Empty Party workflow](https://console.invopop.com/redirect/workflows/new?template=empty-party) code view.
...
<Card title="Participate in our community"
  ...
  horizontal
>
  Ask and answer questions about suppliers in Spain →
</Card>

...
ES-00003
```
The code (`INV`) is an internal identifier and does not appear in the generated sequential code.
...
```json
{
  ...
  "args": {
    "sequence-series-id": "835bb1ca-e7c4-41e6-9e88-464de54cc08e"
  }
}
```
...
- If `sequence-series-id` is not provided or is empty, the workflow step falls back to its normal behavior (dynamic or static series from the configuration).
...
The `code` field in the response contains the generated sequential code. You can then use this code in your document's `code` field. The series object does not report its current position; to find the latest number, list the series entries, which are returned newest first.
...
Generated codes: `ES-00001`, `ES-00002`, `ES-00003`, ... (the `INV` code is internal and never appears in the output)
...
The **Code** field is an optional internal identifier for the series (stored in the `code` property). The generated sequential code (returned in the `code` field of an entry) is the formatted string that includes prefix, padded number, and suffix (e.g., `ES-00001`).
</Accordion>

<Accordion title="Can I modify a series after creating it?">
You can update some series properties via the API, but be cautious as changes may affect existing entries. The `start` value only applies before the first entry is created.
</Accordion>
</AccordionGroup>

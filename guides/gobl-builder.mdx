...
GOBL Builder is a browser-based, interactive editor that lets you create, edit, validate and build documents compliant with [GOBL](/guides/gobl) — a JSON-Schema-based standard for business documents ([invoices](/console/doc-schemas), messages, etc.).
...
* **Menu bar / toolbar** — top-level controls (open, save, choose schema).
...
* **Schema selector / configuration input** — ability to choose the kind of [GOBL](/guides/gobl) document you are building ([invoice](/console/doc-schemas), message, etc.), by pointing to a JSON-Schema URL, so that the tool knows how to interpret and validate your data.
...
* **Create [GOBL](/guides/gobl) documents from scratch** — define your business document ([invoice](/console/doc-schemas), message, etc.) in JSON or YAML following the appropriate schema.
...
* **Experiment with different schemas easily** — switch between [document types](/console/doc-schemas) by changing the schema URL; handy for exploring what's possible without setting up a dev environment.
...
* For **learning / exploring** [GOBL](/guides/gobl) — experimenting with different schemas to see how [GOBL](/guides/gobl) structures different [document types](/console/doc-schemas).

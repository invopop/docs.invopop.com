...
import WorkflowExample, { grIaprInvoiceWorkflow } from '/snippets/workflows/gr/iapr-invoice.mdx';
...
myDATA identifies each party by tax ID and branch number. The branch number is the establishment number declared in the AADE taxpayer registry. `0` is the headquarters.

By default, Invopop reports branch `0` for both parties. To report a different branch, set the [`gr-mydata-branch`](https://docs.gobl.org/addons/gr-mydata-v1#branch-number) extension on the party:
...
The extension works on both `supplier` and `customer`. The value must be a number between `0` and `9999`, with no leading zeros. It must match an establishment declared for the party in the AADE registry; myDATA may reject submissions with unknown branch numbers.

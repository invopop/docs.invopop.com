...
To issue service notes in Greece, you will need:
...
- Have authorized "ILYDA", the provider used by Invopop, to issue invoices on the supplier's behalf in the IAPR, and followed the specific on-boarding process (details below).

## Supplier on-boarding

<Warning>
  Supplier on-boarding is only possible in production. In the sandbox environment, your invoices must use a supplier with the tax ID code set to `177472438` which is the TIN of the test supplier pre-enabled in that environment.
</Warning>
...
Don't hesitate to contact us via Slack or [support@invopop.com](mailto:support@invopop.com) whenever you need to on-board a new supplier, and we will walk you through the setup process.
...
The following steps must be carried out from the [Invopop Console](https://console.invopop.com).
...
Finally, we need to prepare the workflow that will be used to process invoices. Head over to the **Workflows** section and create a new "Invoice" workflow.
...
In this example, we're issuing a simple service note from a Greek supplier to another Greek customer.
...
Please contact the [Invopop support team] (email:support@invopop.com) if you have any questions not listed here.

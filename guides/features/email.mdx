...
## Custom Variables in Job Args

In addition to the predefined variables available in the UI, you can inject custom data into your email templates using job args. This is particularly useful for dynamic content like payment links, custom messages, or other external data.

### How It Works

Custom variables follow this pattern:
- **Template syntax**: `{{args.variable_name}}` in your email body or subject
- **API payload**: Include the variable in the `args` object when executing the job

### Example: Adding a Payment Link

Let's say you want to send an email with a custom payment link. Here's how to set it up:

**Step 1: Configure your email template**

In the email body field, use both UI variables (displayed as clickable tags like `Customer Name` and `Full Code`) and custom variables (in `{{args.name}}` format):

```
Hello [Customer Name],

Your invoice [Full Code] is attached to this email.

You can pay it here: {{args.payment_link}}

Best regards,
Invopop
```

**Step 2: Execute the job with custom args**

When calling the API to run the workflow, include your custom variables in the `args` field:
...
**Result**: The customer receives this email:

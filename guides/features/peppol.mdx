...
You can perform these steps manually via the Console or programmatically via the API using the [Create an Entry](/api-ref/silo/entries/create-an-entry-put.mdx) and [Create a Job](/api-ref/transform/jobs/create-a-job-post.mdx) endpoints. For API usage, refer to our [API Reference](http://localhost:3000/api-ref/introduction).

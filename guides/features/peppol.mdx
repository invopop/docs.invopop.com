...
<Frame>
[Screenshot of webform will be added here]
</Frame>
...
For programmatic integration, you can use our API endpoints instead of the webform. This is particularly useful for white-label implementations where you want to integrate the registration process into your own application.

The process involves:
1. Upload the proof of ownership document using the API
2. Call the confirm endpoint to start the validation process

These endpoints are described in detail in the [API reference](#) (link to be added).
...
1. Store it as a [Silo Entry](/api-ref/silo/entries/create-an-entry-put.mdx) with the Peppol inboxes previously obtained.
...
- `smp+sml`: SMP and SML, usefull when you want to send documents but not receive them
- `smp+sml+peppol`: SMP, SML, and Peppol Directory, recomended for both sending and receiving documents as you will be visible in the Peppol Directory

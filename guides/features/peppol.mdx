...
Use the [Console](https://console.invopop.com/) to build a workflow of the Invoice type and include the _Import Peppol Document_ step which fetches the document from the Peppol network. You also need to add a step to parse the incoming document, for example, _Import UBL Document_ for UBL documents. Consider adding a _Send Webhook_ step at the end to receive notifications for incoming invoices.

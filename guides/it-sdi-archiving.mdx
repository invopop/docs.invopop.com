...
- `ERR` when the preservation service is temporarily unavailable — the platform retries, and the step resumes from where it stopped
- `KO` when there is no FatturaPA file on the entry (the invoice was never exchanged with SDI, or the step ran before the send or import step), when the preservation service rejects the invoice's metadata, when the account is not authorised for preservation, or when the same file was already archived from another entry

...
The step reads the company's Italian tax ID from the party, so the party must carry one. Re-running it on an already-enabled company is skipped: the step reports `Skipped` without contacting the preservation service again.
...
- `Skipped` when the entry already carries the receipt — an earlier run archived the invoice
...
| The step shows an error, then succeeds on a later attempt | The preservation service was briefly unavailable. The step resumed from where it stopped, without uploading the document again. |

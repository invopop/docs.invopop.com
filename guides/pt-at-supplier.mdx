...
import PostRegistration, { ptAtPostregistrationWorkflow } from '/snippets/workflows/pt/at-postregistration.mdx';
import RegisterSupplier, { ptAtRegisterSupplierWorkflow } from '/snippets/workflows/pt/at-register-supplier.mdx';
import UnregisterSupplier, { ptAtUnregisterSupplierWorkflow } from '/snippets/workflows/pt/at-unregister-supplier.mdx';
...
In this guide you'll learn how to register a supplier using the [AT Portugal app](/apps/at-portugal), keep their data up to date, manage their document series, and unregister them when they should no longer issue invoices from your workspace. Once a supplier is registered, continue with the companion guide: [Portugal: Issuing invoices](/guides/pt-at).

| - | Sandbox | Live |
|---|---|---|
| **Supplier** | Real company or self-employed details required | Real company or self-employed details required |
...
Find the **Parties** section of the sidebar and click **Suppliers**. Click the **+ New Supplier** button to be presented with a new editor. You can use the following example as a template, and replace it with the details of your company. Note that you'll need the details of a real company or self-employed individual both in the sandbox and production environments:

  <Tabs>
    <Tab title="Company">
      <Supplier />
    </Tab>
    <Tab title="Self-employed individual">
      A self-employed individual (_trabalhador independente_) is identified by their personal NIF, full name and address.
...
Unregistering a supplier revokes their invoice issuing privileges from your workspace. You might need to unregister an entity because the supplier is no longer your client or because of a change in their Tax ID. To do so, use the following workflow template:
  ...
  <Card title="AT unregister supplier" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=pt-at-unregister-supplier" horizontal>
    Add to my workspace →
  </Card>
  <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={ptAtUnregisterSupplierWorkflow} />
      </Tab>
      <Tab title="Code">
        Copy and paste into a new [Empty Party workflow](https://console.invopop.com/redirect/workflows/new?template=empty-party) code view.
...
<Info>
  Unregistering also releases the supplier's [Supplier Portal](https://at-pt.invopop.com/portal) email address, so it becomes available again for registering a supplier in the same or in any other workspace.
</Info>

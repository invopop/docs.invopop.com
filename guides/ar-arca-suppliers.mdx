...
import ArRegistrationWorkflow from '/snippets/workflows/ar/ar-supplier-registration.mdx'
import ArgentinaResources from '/snippets/tables/argentina-resources.mdx'
import FAQ from '/snippets/faqs/ar/composers/guide-arca-supplier.mdx';
import { WorkflowDiagram } from '/snippets/components/workflow.jsx';
import { arSupplierRegistrationWorkflow } from '/snippets/workflows/ar/ar-supplier-registration-data.mdx';
...
<Steps>
 <Step title="Connect the Argentina app">
  ...
  <Step title="Create a supplier registration workflow">
    ...
    <Card title="ARCA supplier registration workflow" icon="code-branch" iconType="duotone" href="https://console.invopop.com/redirect/workflows/new?template=ar-supplier-registration" horizontal>
      Add to my workspace →
    </Card>
    <Tabs>
      <Tab title="Workflow">
        <WorkflowDiagram workflow={arSupplierRegistrationWorkflow} />
      </Tab>
      <Tab title="Code">
...
The registration process is the same whether you're issuing invoices on behalf of third parties ([white label](/guides/white-label)) or under your own company name.

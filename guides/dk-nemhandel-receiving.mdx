...
- **Invoice responses become status entries.** An OIOUBL ApplicationResponse — the message a customer sends back accepting or rejecting an invoice — is imported as its own `bill/status` silo entry, with the original XML attached. The import job's result carries the outcome (`accepted`, `rejected`, …), and the workflow template above branches on it to mark the entry's state.

...
<Prompt description="Have your agent plan a white-label integration for your platform." icon="tag" actions={["copy", "cursor"]}>
Read https://docs.invopop.com/workspace/white-label.md, https://docs.invopop.com/workspace/multi-country.md and https://docs.invopop.com/llms.md, then the supplier registration guide and the invoicing guide for each country I name. Produce an integration plan for issuing invoices on behalf of my customers: one workspace per tax regime and how requests are routed to them; the onboarding flow per customer as an org/party entry plus the country's registration workflow, listing what each country needs from the customer; the invoice loop of create entry, create job, receive webhook and fetch results, with idempotent UUIDs; how corrections work in each regime; and how totals shown in my UI stay consistent with GOBL by calling the build endpoint. Flag every point where my product must collect a credential or a signature from the customer.
</Prompt>

...
Let's examine the process of issuing a new invoice from the perspective of a receptionist using Hotelpop.
...
When ready to issue the invoice, the receptionist clicks **Send Invoice.** Hotelpop converts the invoice to GOBL and uploads it using the [**Create an Entry**](/api-ref/silo/entries/create-an-entry-put) endpoint, which requires a unique UUID from Hotelpop.
...
The **Create an Entry** endpoint is synchronous, typically responding in milliseconds. Consequently, when the receptionist presses <kbd>Send Invoice</kbd>, Hotelpop can wait for Invopop's feedback, for example, with a spinner on the button or a "pressed" animation on the button.

**Successful Upload**

After successful storage, Hotelpop sends the entry to an Invopop workflow using the [**Create a Job**](/api-ref/transform/jobs/create-a-job-put) endpoint. Each workflow instance is a "job" that handles tasks like format conversion, tax authority submission, and webhook triggers.
...
After creating a job, Hotelpop waits for the webhook and then uses the Silo Entry ID included in it to fetch the state from the corresponding Silo entry (Step 4). This is our recommended approach.
...
After receiving the webhook, fetch the [**Silo Entry**](/api-ref/silo/entries/fetch-an-entry) to update invoice status, show errors, and manage output files.
...
## Correcting invoices
Correcting an invoice follows a similar process to issuing a new one: you upload a document to the Silo and send it to a workflow. The key difference is that this document will be a [GOBL](https://docs.gobl.org) document of `credit-note` type. A credit note contains all fields from the original invoice plus a `preceding` field that references the original invoice.
...
<Frame>
  ...
  />
    ...
    ```json Invoice example in Germany expandable
    {
...
Every time Hotelpop onboards a new customer, they configure their company details (name, tax ID, address) in a configuration page in Hotelpop. When the hotel clicks on "Enable invoicing" in Hotelpop, the system creates an [org.party](https://docs.gobl.org/draft-0/org/party) [GOBL](https://docs.gobl.org) document, uploads it to Invopop, and sends it to a registration workflow.
...
In Step 1, we discussed that most invoice errors are caught by [GOBL](https://docs.gobl.org) validations and returned to you synchronously when uploading the invoice. At Invopop, we put a lot of effort into including as many validations in GOBL as possible to catch errors early.
...
While option 1 is simpler, some jurisdictions, especially those with fiscalization requirements (e.g., TicketBAI, VERI\*FACTU, Portugal), do not allow invoice modifications past a certain stage.
...
1. **Match GOBL's algorithms:** Invopop's calculation algorithm is included in the open-source [GOBL](https://docs.gobl.org) library. Hotelpop can use this information to align their algorithm with GOBL's implementation.
2. **Use the Build GOBL Document endpoint:** Invopop provides a [Build GOBL Document](/api-ref/silo/gobl/build) endpoint that validates and calculates all invoice fields. Hotelpop can call this endpoint when calculating draft invoices to ensure accurate calculations.
3. **Implement the GOBL library:** Hotelpop can integrate the [GOBL](https://docs.gobl.org) library directly into their system for all calculations. This approach is particularly beneficial for systems using Go internally.

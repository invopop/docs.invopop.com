...
Invopop supports a set of business document types based on the [GOBL (Go Business Language)](https://docs.gobl.org) schema. All documents are stored as GOBL Envelopes, which wrap the document data with headers and digital signatures for integrity and authenticity.
...
There are some other regime specific invoice types such as simplified invoices that are supported through invoice scenarios. [Read more about simplified invoices](https://docs.gobl.org/use-cases/invoicing#invoice-scenarios) in the GOBL docs.

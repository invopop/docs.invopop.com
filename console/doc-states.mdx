...
States are labels that do not reflect the internal state of the document. This decoupling means the state system provides metadata about workflow events, not assertions about the document's validity or integrity.
...
In the API these two system labels are not values of the entry's `state` property. A freshly created entry has no `state` key at all, which the Console shows as **Empty**, and a document stored with `allow_invalid` carries `invalid: true` and its `faults` on the entry, which the Console shows as **Invalid**. States you set yourself appear as lowercase keys, for example `state: "paid"`.

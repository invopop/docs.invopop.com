...
The Related tab lists the documents Invopop has linked to the current envelope, resolved from the UUID references the documents carry. Which property carries each reference depends on the schema — see [document references](/console/doc-schemas#document-references).

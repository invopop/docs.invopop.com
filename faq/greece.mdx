...
<AccordionGroup>
  ...
  <Accordion title="Why is my credit note being rejected with 'Invoice not found' error?">
    If you issue a credit note immediately after the invoice it refers to, the MyDATA system may reject it because the original invoice hasn't fully synced yet. You'll typically see an error like:

    ```
    Fatal error 301: Invoice with ΜΑΡΚ xxxx not found for VAT number xxxx
    ```

    This happens because there's a brief delay between when an invoice is accepted by MyDATA and when it becomes fully available for reference by other documents like credit notes.

    **Solution**: Wait a few moments (typically 30-60 seconds) after successfully submitting the original invoice before issuing the credit note. This allows the MyDATA system to complete the synchronization process.
  </Accordion>
</AccordionGroup>

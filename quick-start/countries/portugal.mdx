...
Invopop's InvoiceXpress Provider lets you issue invoices, invoice-receipts and credit notes in Portugal using the [InvoiceXpress](https://invoicexpress.com/) service. This guide will walk you through the steps necessary to setup a workflow that will let you issue GOBL documents in Portugal.

Please refer to the docs of the [GOBL's Portugal Tax Regime](https://github.com/invopop/gobl/blob/main/regimes/pt/README.md) to learn about the specifics of Portuguese GOBL documents and get examples.
...
At this point, the InvoiceXpress account will be ready to issue documents. To make that happen, you'll need to send to send [GOBL Invoice](https://docs.gobl.org/draft-0/bill/invoice) documents to the same workflow you created in the previous section, making sure the `supplier`'s `uuid` is the one you set when you sent the GOBL Party document to create the account.

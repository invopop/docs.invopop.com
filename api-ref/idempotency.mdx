...
[^1]: All UUID versions are supported by most operations. However, there are a few instances where only time based UUID versions 1 and 7 are supported. Please check the reference documentation for each create operation to see which UUID versions it supports.

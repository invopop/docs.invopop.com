...
This request will not be idempotent unless a `key` is provided. Keys are checked for uniqueness within the current workspace and are valid for up to 2 years. Note that the [put](create-a-job-put) request is recommend for high concurrency scenarios.
